...
# AGENT: ENTRYPOINT: ASGI app launched directly by `ops/windows/20_start_bridge.bat` through Uvicorn.
...
async def _bridge_lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    if (
        os.environ.get("FXSTACK_SKIP_STARTUP_VALIDATION") or ""
    ...
    try:
        ...
        if callable(drain):
            try:
                ...
                _bridge_logger.exception(
                    "fxstack bridge shutdown: service.drain() raised"
                )
        ...
        if grace_secs > 0:
            try:
                await asyncio.wait_for(
                    asyncio.sleep(grace_secs), timeout=grace_secs + 1.0
                )
            except asyncio.TimeoutError:  # pragma: no cover - belt-and-suspenders
                _bridge_logger.warning(
                    "fxstack bridge shutdown: grace window timed out"
                )
...
async def _bridge_http_exception_handler(
    ...
    if isinstance(exc.detail, dict):
        ...
        message = str(
            detail_payload.pop("message", None)
            or detail_payload.pop("detail", None)
            or f"HTTP {exc.status_code}"
        )
    else:
        message = (
            str(exc.detail) if exc.detail is not None else f"HTTP {exc.status_code}"
        )
    return JSONResponse(
...
def _handshake_build() -> str:
    ...
    from fxstack.api.wire import (
        _build_revision as _rev,
    )  # local import to avoid cycle warnings
...
def _market_source_contract_required() -> bool:
    ...
    return bool(
        str(settings.start_profile or "").strip().lower() == "live" or any(configured)
    )
...
def _authenticate_market_source_ingest(
    ...
    *,
    ...
    if _broker_identity_match_token(broker_server) != _broker_identity_match_token(
        state.get("broker_server")
    ) or _broker_identity_match_token(broker_company) != _broker_identity_match_token(
        state.get("broker_company")
    ):
        raise _market_source_ingest_error(
    ...
    if str(raw.get("broker_account_scope_schema") or "").strip() != str(
        state.get("broker_account_scope_schema") or ""
    ).strip() or int(_safe_float(raw.get("broker_account_scope_version"), 0.0)) != int(
        _safe_float(state.get("broker_account_scope_version"), 0.0)
    ):
        raise _market_source_ingest_error(
...
def _normalize_closed_trade_report(row: dict[str, Any]) -> dict[str, Any] | None:
    payload = _report_payload(
        str(row.get("report_text", "") or ""), row.get("report_json")
    )
    ...
    net_profit = float(
        _safe_float(payload.get("net_profit"), profit + swap + commission)
    )
    normalized = {
        "ticket": ticket,
        "symbol": str(payload.get("symbol") or "").strip().upper(),
        "broker_symbol": str(
            payload.get("broker_symbol") or payload.get("symbol") or ""
        ).strip(),
        ...
        "duration_secs": max(0.0, close_time - open_time)
...
def _decorate_market_event_freshness(
    ...
    *,
    ...
    baseline_initialized = bool(item.get("source_event_baseline_initialized", False))
...
def _tick_liveness(
    *,
    ...
    market_fresh = bool(
        market_event_by_symbol and market_fresh_count == len(market_event_by_symbol)
    )
...
def _feature_serving_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    telemetry = dict(
        (state or {}).get("feature_serving")
        or runtime_diag.get("feature_serving")
        or {}
    )
    feature_serving_by_pair = dict(
        (state or {}).get("feature_serving_by_pair")
        or runtime_diag.get("feature_serving_by_pair")
        or {}
    )
    if not telemetry:
        telemetry = {
...
def _feature_observability_telemetry(
    ...
    feature_push = dict(
        (metrics or {}).get("feature_push")
        or (state or {}).get("feature_push")
        or runtime_diag.get("feature_push")
        or {}
    )
    ...
    feature_push_backlog_ok = bool(
        not feature_push_enabled or feature_push_backlog <= feature_push_warn
    )
    feature_online_ready = bool(
        str(
            (state or {}).get("feature_serving_source")
            or feature_serving.get("source")
            or ""
        ).strip()
    )
    feature_serving_stale = bool(
        (state or {}).get("feature_serving_stale", feature_serving.get("stale", False))
    )
    ...
    feature_serving_by_pair_pairs = sorted(
        {key.split(":", 1)[0] for key in feature_serving_by_pair_keys}
    )
    feature_serving_by_pair_stale_keys = sorted(
        key
        for key, item in feature_serving_by_pair.items()
        if bool(item.get("stale", False))
    )
    feature_serving_by_pair_stale_pairs = sorted(
        {key.split(":", 1)[0] for key in feature_serving_by_pair_stale_keys}
    )
    feature_serving_summary = {
        ...
        "selected_pairs_count": int(
            feature_serving_details.get("selected_pairs_count") or 0
        ),
        "selected_pairs": list(feature_serving_details.get("selected_pairs") or []),
        "selected_timeframes": dict(
            feature_serving_details.get("selected_timeframes") or {}
        ),
        "selected_stale_count": int(
            feature_serving_details.get("selected_stale_count") or 0
        ),
        ...
        "all_stale_timeframes": list(
            feature_serving_details.get("all_stale_timeframes") or []
        ),
    ...
    feature_push_summary = {
        ...
        "overage": max(0, feature_push_backlog - feature_push_warn)
        if feature_push_enabled
        else 0,
    ...
    feature_bar_status = (
        "fresh"
        if feature_data_fresh
        else ("stale" if feature_online_ready else "missing")
    )
    return {
        ...
        "feature_push_backlog_overage": max(0, feature_push_backlog - feature_push_warn)
        if feature_push_enabled
        else 0,
        ...
        "feature_blocker_source": str(
            "feature_serving"
            if blocker_reasons and blocker_reasons[0].startswith("feature_serving")
            else "feature_push"
            if blocker_reasons
            else ""
        ),
...
def _rl_portfolio_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    proposals_by_pair = {
        ...
        for key, value in dict(
            rl_portfolio_proposal.get("proposals_by_pair") or {}
        ).items()
        if str(key).strip()
    }
    pair_universe = [
        str(pair).upper()
        for pair in list(rl_portfolio_proposal.get("pair_universe") or [])
        if str(pair).strip()
    ]
    checkpoint_loaded = bool(
        entry_execution_policy.get(
            "rl_checkpoint_loaded",
            rl_portfolio_proposal.get("checkpoint_loaded", False),
        )
    )
    checkpoint_path = str(
    ...
    proposal_source = str(
        entry_execution_policy.get("rl_proposal_source")
        or rl_portfolio_proposal.get("source")
        or ""
    )
    fallback_reason = str(
        entry_execution_policy.get("rl_fallback_reason")
        or rl_portfolio_proposal.get("fallback_reason")
        or ""
    )
    summary = {
        ...
        "routed_entry_count": int(
            entry_execution_policy.get("rl_routed_entry_count") or 0
        ),
        "blocked_entry_count": int(
            entry_execution_policy.get("rl_blocked_entry_count") or 0
        ),
        "fallback_entry_count": int(
            entry_execution_policy.get("rl_fallback_entry_count") or 0
        ),
        "scaled_entry_count": int(
            entry_execution_policy.get("rl_scaled_entry_count") or 0
        ),
        "lifecycle_reviewed_count": int(
            entry_execution_policy.get("rl_lifecycle_reviewed_count") or 0
        ),
        "lifecycle_applied_count": int(
            entry_execution_policy.get("rl_lifecycle_applied_count") or 0
        ),
        "lifecycle_exit_count": int(
            entry_execution_policy.get("rl_lifecycle_exit_count") or 0
        ),
        "lifecycle_flip_exit_count": int(
            entry_execution_policy.get("rl_lifecycle_flip_exit_count") or 0
        ),
        "lifecycle_resize_count": int(
            entry_execution_policy.get("rl_lifecycle_resize_count") or 0
        ),
        "lifecycle_tighten_stop_count": int(
            entry_execution_policy.get("rl_lifecycle_tighten_stop_count") or 0
        ),
        "lifecycle_preserved_exit_count": int(
            entry_execution_policy.get("rl_lifecycle_preserved_exit_count") or 0
        ),
        "lifecycle_fallback_count": int(
            entry_execution_policy.get("rl_lifecycle_fallback_count") or 0
        ),
        "lifecycle_pairs": [
            str(pair).upper()
            for pair in list(entry_execution_policy.get("rl_lifecycle_pairs") or [])
            if str(pair).strip()
        ],
        ...
        "proposal_count": int(
            proposal_diagnostics.get("decision_count") or len(proposals_by_pair)
        ),
...
def _rl_lifecycle_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    entry_execution_policy = dict(
        runtime_diag.get("entry_execution_policy")
        or (state or {}).get("rl_execution_policy")
        or (state or {}).get("rlExecutionPolicy")
        or {}
    )
    proposal_diagnostics = dict(rl_portfolio_proposal.get("diagnostics") or {})
    checkpoint_summary = dict(
        rl_portfolio_proposal.get("checkpoint_summary")
        or proposal_diagnostics.get("checkpoint_summary")
        or {}
    )
    ...
    lifecycle_summary = {
        "checkpoint_loaded": bool(
            entry_execution_policy.get(
                "rl_lifecycle_checkpoint_loaded",
                rl_portfolio_proposal.get("checkpoint_loaded", False),
            )
        ),
        "proposal_source": str(
            entry_execution_policy.get("rl_lifecycle_proposal_source")
            or rl_portfolio_proposal.get("source")
            or ""
        ),
        "reviewed_count": int(
        ...
        "tighten_stop_count": int(
            entry_execution_policy.get("rl_lifecycle_tighten_stop_count") or 0
        ),
        "preserved_exit_count": int(
            entry_execution_policy.get("rl_lifecycle_preserved_exit_count") or 0
        ),
        "fallback_count": int(
            entry_execution_policy.get("rl_lifecycle_fallback_count") or 0
    ...
    return {
        ...
        "artifact_readiness": {
            ...
            "checkpoint_loaded": bool(
                artifact_discovery.get(
                    "checkpoint_loaded",
                    rl_portfolio_proposal.get("checkpoint_loaded", False),
                )
            ),
            "checkpoint_path": str(
                artifact_discovery.get("checkpoint_path")
                or rl_portfolio_proposal.get("checkpoint_path")
                or ""
            ),
            "fallback_reason": str(
                artifact_discovery.get("fallback_reason")
                or rl_portfolio_proposal.get("fallback_reason")
                or ""
            ),
        ...
        "proposal_source": str(
            rl_portfolio_proposal.get("source") or lifecycle_summary["proposal_source"]
        ),
        "supervised_fallback_used": bool(
            rl_portfolio_proposal.get("supervised_fallback_used", False)
        ),
...
def _pair_readiness_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    feature_serving_by_pair = {
        ...
        for key, item in dict(
            (
                state.get("feature_serving_by_pair")
                or runtime_diag.get("feature_serving_by_pair")
                or {}
            )
        ).items()
    ...
    pair_keys = sorted(
        ...
        | {
            str(key).upper()
            for key in dict(model_load.get("pairs") or {})
            if str(key).strip()
        }
    )
    out: dict[str, dict[str, Any]] = {}
    for pair in pair_keys:
        ...
        feature_serving = _pair_feature_serving_snapshot(
            pair=pair, feature_serving_by_pair=feature_serving_by_pair
        )
        ...
        if not startup:
            ...
            blockers.append(
                f"startup_inference:{str(startup.get('reason') or 'blocked')}"
            )
        if feature_serving:
        ...
        if symbol and not bool(symbol.get("supported", True)):
            blockers.append(
                f"symbol_readiness:{str(symbol.get('broker_symbol') or 'unsupported')}"
            )
        ...
        if str(model_pair.get("failure_reason") or "").strip():
            blockers.append(
                f"model_load:{str(model_pair.get('failure_reason') or 'error')}"
            )
        out[pair] = {
            "pair": pair,
...
def _provider_health_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not provider_roles:
        provider_roles = {
            "history_provider": str(
                getattr(settings, "normalized_history_provider", "") or ""
            ),
            "market_data_provider": str(
                getattr(settings, "normalized_market_data_provider", "") or ""
            ),
            "execution_provider": str(
                getattr(settings, "normalized_execution_provider", "") or ""
            ),
        }
    if not provider_health:
        ...
        mt4_status = (
            str((state or {}).get("system_status") or "unknown").strip().lower()
        )
        mt4_fresh = bool(
        ...
        market_data_provider_name = str(
            provider_roles.get("market_data_provider") or ""
        )
        ...
        execution_shadow_only = bool(
            str(execution_provider_name).strip().lower() != "mt4"
        )
        provider_health = {
            "history_provider": {
            ...
            "execution_provider": {
                ...
                "role": "execution",
                "status": "ok"
                if execution_shadow_only or bool(mt4_fresh)
                else "degraded",
    ...
    history_provider_name = str(
        history_provider.get("provider") or provider_roles.get("history_provider") or ""
    )
    market_data_provider_name = str(
        market_data_provider.get("provider")
        or provider_roles.get("market_data_provider")
        or ""
    )
    execution_provider_name = str(
        execution_provider.get("provider")
        or provider_roles.get("execution_provider")
        or ""
    )
    source_chain = [
        item
    ...
    return {
        ...
        "primary_provider": market_data_provider_name
        or history_provider_name
        or execution_provider_name,
...
def _capital_governance_telemetry(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not telemetry:
        telemetry = {
            "capital_band": str(
                getattr(settings, "capital_band_mode", "paper") or "paper"
            ),
            "mode": "shadow_only"
            if bool(getattr(settings, "provider_shadow_only", False))
            else "normal",
            "paused": False,
    ...
    rollback_armed = any(
        bool(dict(item or {}).get("armed", False)) for item in rollback_actions
    )
    rollback_reason = ""
...
def _runtime_startup_summary(state: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime_status = (
        str((state or {}).get("runtime_status") or "unknown").strip().lower()
    )
    ...
    startup_inference_failures = int(
        runtime_diag.get("startup_inference_failures", 0) or 0
    )
    ...
    return {
        ...
        "failure_component": str(
            runtime_startup.get("failure_component") or ""
        ).strip(),
        ...
        "pending_command_policy": str(
            runtime_startup.get("pending_command_policy") or ""
        ).strip(),
...
def _latest_runtime_startup_failure(
    ...
    for event in list(events or []):
        ...
        failed_at_raw = (
            payload.get("failed_at") or row.get("failed_at") or row.get("ts")
        )
        ...
        return {
            ...
            "failedAgeSecs": max(0.0, _utc_now_ts() - failed_at)
            if failed_at > 0
            else None,
        }
    return None
...
def _runtime_startup_failure_history(
...
def _state_with_liveness(
    raw: dict[str, Any],
    *,
    include_decision_snapshot: bool = False,
    latest_decision_diagnostics: dict[str, Any] | None = None,
    metrics: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if include_decision_snapshot and latest_decision_diagnostics is not None:
        raise ValueError("decision_snapshot_modes_are_mutually_exclusive")
    state = dict(raw or {})
    ...
    runtime_startup_progress_stale_secs = max(
        1.0, float(settings.runtime_startup_progress_stale_secs)
    )
    ...
    runtime_phase_pair = (
        str(raw_runtime_startup.get("phase_pair") or "").strip().upper()
    )
    ...
    runtime_failure_component = str(
        raw_runtime_startup.get("failure_component") or ""
    ).strip()
    runtime_failure_pair = str(raw_runtime_startup.get("failure_pair") or "").strip()
    runtime_failure_reason = str(
        raw_runtime_startup.get("failure_reason") or ""
    ).strip()
    ...
    normalized_runtime_startup = {
        ...
        "pending_command_policy": str(
            raw_runtime_startup.get("pending_command_policy") or ""
        ).strip(),
    }
    ...
    state["runtime_startup_warning_count"] = int(
        runtime_startup_summary.get("warning_count") or 0
    )
    state["runtimeStartupWarningCount"] = int(
        runtime_startup_summary.get("warning_count") or 0
    )
    state["model_load_errors"] = int(
        runtime_startup_summary.get("model_load_errors") or 0
    )
    state["model_load_timeouts"] = int(
        runtime_startup_summary.get("model_load_timeouts") or 0
    )
    state["startup_inference_failures"] = int(
        runtime_startup_summary.get("startup_inference_failures") or 0
    )
    state["startup_disabled_pairs"] = list(
        runtime_startup_summary.get("startup_disabled_pairs") or []
    )
    ...
    state.update(feature_observability)
    state["feature_observability"] = dict(feature_observability)
    state["featureObservability"] = dict(state["feature_observability"])
    state["feature_bar_status"] = str(
        state["feature_observability"].get("feature_bar_status") or ""
    )
    ...
    state["feature_serving_feature_service"] = str(
        feature_serving.get("feature_service") or ""
    )
    ...
    state["rl_checkpoint_loaded"] = bool(
        rl_portfolio_proposal.get("checkpoint_loaded", False)
    )
    state["rlCheckpointLoaded"] = bool(
        rl_portfolio_proposal.get("checkpoint_loaded", False)
    )
    state["rl_checkpoint_path"] = str(
        rl_portfolio_proposal.get("checkpoint_path") or ""
    )
    state["rlCheckpointPath"] = str(rl_portfolio_proposal.get("checkpoint_path") or "")
    state["rl_proposal_source"] = str(
        rl_portfolio_proposal.get("proposal_source") or ""
    )
    state["rlProposalSource"] = str(rl_portfolio_proposal.get("proposal_source") or "")
    state["rl_supervised_fallback_used"] = bool(
        rl_portfolio_proposal.get("supervised_fallback_used", False)
    )
    state["rlSupervisedFallbackUsed"] = bool(
        rl_portfolio_proposal.get("supervised_fallback_used", False)
    )
    state["rl_fallback_reason"] = str(
        rl_portfolio_proposal.get("fallback_reason") or ""
    )
    state["rlFallbackReason"] = str(rl_portfolio_proposal.get("fallback_reason") or "")
    state["rl_routed_entry_count"] = int(
        rl_portfolio_proposal.get("routed_entry_count") or 0
    )
    state["rlRoutedEntryCount"] = int(
        rl_portfolio_proposal.get("routed_entry_count") or 0
    )
    state["rl_blocked_entry_count"] = int(
        rl_portfolio_proposal.get("blocked_entry_count") or 0
    )
    state["rlBlockedEntryCount"] = int(
        rl_portfolio_proposal.get("blocked_entry_count") or 0
    )
    state["rl_fallback_entry_count"] = int(
        rl_portfolio_proposal.get("fallback_entry_count") or 0
    )
    state["rlFallbackEntryCount"] = int(
        rl_portfolio_proposal.get("fallback_entry_count") or 0
    )
    state["rl_scaled_entry_count"] = int(
        rl_portfolio_proposal.get("scaled_entry_count") or 0
    )
    state["rlScaledEntryCount"] = int(
        rl_portfolio_proposal.get("scaled_entry_count") or 0
    )
    ...
    state["runtime_model_load_failures"] = int(
        len(model_load.get("failed_pairs") or [])
    )
    state["runtime_model_load_failed_pairs"] = list(
        model_load.get("failed_pairs") or []
    )
    state["runtime_model_load_degraded_pairs"] = list(
        model_load.get("degraded_pairs") or []
    )
    ...
    if raw_runtime_status == "running" and (
        runtime_cycle_age_secs is None or float(runtime_cycle_age_secs) > 30.0
    ):
    ...
    strategy_engine_mode = str(
        runtime_diag.get("strategy_engine_mode") or "supervised_legacy"
    )
    ...
    state["canary_active"] = bool(
        int(rollout_policy.get("active_count") or 0) > 0
        or int(risk_cycle_summary.get("rollout_active_count") or 0) > 0
    )
    state["canary_pairs"] = list(
        rollout_runtime.get("active_pairs") or rollout_policy.get("active_pairs") or []
    )
    ...
    if symbol_readiness:
        broker_symbol_failures = sorted(
            [
                ...
                if not bool(dict(symbol_readiness.get(pair) or {}).get("supported"))
            ]
        )
        broker_symbol_ready_count = int(
            sum(
                1
                for pair in configured_pairs
                if bool(dict(symbol_readiness.get(pair) or {}).get("supported"))
            )
        )
    else:
        tick_pairs = {str(pair).upper() for pair in _market_ticks_mem.keys()}
        broker_symbol_failures = sorted(
            [pair for pair in configured_pairs if pair not in tick_pairs]
        )
        broker_symbol_ready_count = int(
            sum(1 for pair in configured_pairs if pair in tick_pairs)
        )
    ...
    state["activation_mismatch_pairs"] = list(
    ...
    state["active_registry_root"] = str(
        activation_consistency.get("active_registry_root") or ""
    )
    ...
    if include_decision_snapshot:
        try:
            snapshots = service.get_decision_snapshots(limit=1)
        except Exception:
            snapshots = []
        if snapshots:
            latest_decision_snapshot = dict(snapshots[0] or {})
    elif latest_decision_diagnostics is not None:
        latest_decision_snapshot = dict(latest_decision_diagnostics or {})

    if latest_decision_snapshot:
        if include_decision_snapshot:
            agent_decisions = list(
                latest_decision_snapshot.get("decisions_json") or []
            )
            state["agent_decisions"] = agent_decisions
            state["vol"] = float(latest_decision_snapshot.get("vol") or 0.0)
            state["agent_decisions_source"] = "decision_snapshots"
            state["agent_decisions_ts"] = latest_decision_snapshot.get("ts")
        state["agent_diagnostics"] = dict(
            latest_decision_snapshot.get("diagnostics_json") or {}
        )
        state["agent_diagnostics_source"] = "decision_snapshots"
        state["agent_diagnostics_ts"] = latest_decision_snapshot.get("ts")
    ...
    state["positions_stale"] = bool(
        not mt4_fresh and list(state.get("positions") or [])
    )
    ...
    if not mt4_fresh:
        state["equity"] = 0.0
        if state.get("equity_source") in {
            "runtime_seed",
            "runtime_constant",
            "seed",
        } or not state.get("equity_source"):
    ...
    state["signal_data_fresh"] = bool(
        mt4_fresh and bool(tick_state.get("ticks_fresh")) and runtime_signal_fresh
    )
    return state
...
def _paper_execution_summary(
    *,
    ...
    enabled = bool(
        agent_mode == "paper" or execution_provider == "paper" or paper_commands
    )
    ...
    event_payload = dict(
        latest_event.get("event_json") or latest_event.get("payload_json") or {}
    )
    ...
    orphan_count = sum(
        ...
        in {"queued", "delivered"}
        and not (
            event_statuses_by_command.get(str(dict(item or {}).get("command_id") or ""))
            or set()
        )
    ...
    return {
        ...
        "last_command": {
            ...
            "run_id": str(
                dict(latest_command.get("orchestration_meta_json") or {}).get("run_id")
                or ""
            ),
            "trace_id": str(
                dict(latest_command.get("orchestration_meta_json") or {}).get(
                    "trace_id"
                )
                or ""
            ),
...
def _production_scalp_entry_readiness(state: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime_diag = dict((state or {}).get("runtime_diag") or {})
    agent_diagnostics = dict((state or {}).get("agent_diagnostics") or {})
    production_scalp = dict(
        runtime_diag.get("production_scalp")
        or agent_diagnostics.get("production_scalp")
        or {}
    )
    applicable = (
    ...
    configured_symbols = (
        [str(symbol or "").strip().upper() for symbol in raw_configured_symbols]
    ...
    if not isinstance(raw_configured_symbols, list):
        ...
        if len(configured_symbols) != len(set(configured_symbols)):
            validation_reasons.append("production_scalp_configured_symbols_duplicated")
    ...
    if not isinstance(raw_global_reasons, list):
        ...
        entry_global_reasons = [
            str(reason).strip() for reason in raw_global_reasons if str(reason).strip()
        ]
...
def _live_entry_readiness(
    *,
    ...
    broker_account_mode = (
        str((state or {}).get("broker_account_mode") or "unknown").strip().lower()
    )
    ...
    uncertainty_blocked_symbols = {
        ...
        for symbol in list((execution_uncertainty or {}).get("blocked_symbols") or [])
    ...
    if (
        ...
        for raw_item in list(production_scalp_readiness.get("symbol_readiness") or []):
        ...
        scoped_blocking_reasons = [
            str(reason)
            for reason in list(production_scalp_readiness.get("blocking_reasons") or [])
            if str(reason) != "production_scalp_no_pair_execution_ready"
        ]
        if not scoped_any_ready:
            scoped_blocking_reasons.append("production_scalp_no_pair_execution_ready")
    ...
    if not production_scalp_applicable and not bool(
        (state or {}).get("signal_data_fresh", False)
    ):
        reasons.append("signal_data_stale")
    if production_scalp_applicable:
        reasons.extend(list(production_scalp_readiness.get("blocking_reasons") or []))
    ...
    if bool((execution_uncertainty or {}).get("blocked", False)):
        ...
        uncertain_command_ids = {
            ...
            for command in list((execution_uncertainty or {}).get("commands") or [])
        ...
        if pending_command_ids is not None:
            ...
            covered_pending_count = len(normalized_pending_ids & uncertain_command_ids)
            pending_outside_uncertainty = (
                bool(normalized_pending_ids - uncertain_command_ids)
        ...
        if orphan_command_ids is not None:
            ...
            covered_orphan_count = len(normalized_orphan_ids & uncertain_command_ids)
            orphan_outside_uncertainty = (
                bool(normalized_orphan_ids - uncertain_command_ids)
    ...
    return {
        ...
        "execution_uncertainty_blocked": bool(
            (execution_uncertainty or {}).get("blocked", False)
        ),
        "execution_uncertainty_present": bool(
            (execution_uncertainty or {}).get("present", False)
        ),
...
def _orchestration_live_summary(
    *,
    ...
    raw_runtime_agent_mode = (
        live_diag.get("mode") if "mode" in live_diag else live_diag.get("agent_mode")
    )
    ...
    live_commands = [
        ...
        for item in list(commands or [])
        if str(
            (dict(item or {}).get("orchestration_meta_json") or {}).get("agent_mode")
            or ""
        )
        .strip()
        .lower()
    ...
    return {
        ...
        "entry_ratio_vs_baseline": float(
            live_diag.get("entry_ratio_vs_baseline") or 0.0
        ),
        "entry_ratio_evaluable": bool(live_diag.get("entry_ratio_evaluable", False)),
        "entry_ratio_status": str(
            live_diag.get("entry_ratio_status") or "insufficient_evidence"
        ),
        "entry_ratio_approved_count": int(
            live_diag.get("entry_ratio_approved_count") or 0
        ),
        "entry_ratio_submitted_count": int(
            live_diag.get("entry_ratio_submitted_count") or 0
        ),
        "entry_ratio_accepted_count": int(
            live_diag.get("entry_ratio_accepted_count") or 0
        ),
        "live_command_admission": dict(live_command_admission),
        "entry_configuration_ready": bool(
            entry_readiness.get("configuration_ready", False)
        ),
        ...
        "broker_account_mode": str(
            entry_readiness.get("broker_account_mode") or "unknown"
        ),
        "broker_account_scope_attested": bool(
            entry_readiness.get("broker_account_scope_attested", False)
        ),
        "expected_account_mode": str(
            entry_readiness.get("expected_account_mode") or ""
        ),
        "signal_data_fresh": bool(entry_readiness.get("signal_data_fresh", False)),
        "execution_uncertainty_blocked": bool(
            entry_readiness.get("execution_uncertainty_blocked", False)
        ),
        "execution_uncertainty_present": bool(
            entry_readiness.get("execution_uncertainty_present", False)
        ),
        "execution_uncertainty_scope_contained": bool(
            entry_readiness.get("execution_uncertainty_scope_contained", False)
        ),
        "execution_uncertainty_blocked_symbols": list(
        ...
        "production_scalp_readiness_diagnostics_valid": bool(
            entry_readiness.get("production_scalp_readiness_diagnostics_valid", False)
        ),
        "production_scalp_any_pair_ready": bool(
        ...
        "slot_utilisation_vs_baseline": float(
            live_diag.get("slot_utilisation_vs_baseline") or 0.0
        ),
        "drawdown_deterioration_pct": float(
            live_diag.get("drawdown_deterioration_pct") or 0.0
        ),
        "repeated_graph_fault_count": int(
            live_diag.get("repeated_graph_fault_count") or 0
        ),
        "trace_persistence_failure_count": int(
            live_diag.get("trace_persistence_failure_count") or 0
        ),
...
def _orchestration_live_health_summary(
    ...
    *,
    ...
    runtime_status_text = (
        str(runtime_status or live.get("runtime_status") or "").strip().lower()
    )
    ...
    trace_persistence_failure_count = int(
        live.get("trace_persistence_failure_count") or 0
    )
    ...
    if live_mode and not bool(live.get("new_entry_ready", False)):
        blockers.extend(
            str(reason)
            for reason in list(
                live.get("new_entry_blocking_reasons") or ["new_entry_not_ready"]
            )
            if str(reason).strip()
        )
    ...
    return {
        ...
        "new_entry_blocking_reasons": list(
            live.get("new_entry_blocking_reasons") or []
        ),
        "production_scalp_readiness_applicable": production_scalp_applicable,
        "production_scalp_readiness_diagnostics_valid": bool(
...
def _ready_payload() -> dict[str, Any]:
    (
        raw_state,
        raw_metrics,
        latest_decision_diagnostics,
    ) = service.get_state_metrics_and_latest_decision_diagnostics()
    metrics = dict(raw_metrics or {})
    state = _state_with_liveness(
        raw_state,
        latest_decision_diagnostics=latest_decision_diagnostics,
        metrics=metrics,
    )
    health = dict(service.get_health() or {})
    feature_observability = dict(state.get("feature_observability") or {})
    ...
    runtime_startup_failure_history = _runtime_startup_failure_history(
        governance_events
    )
    ...
    runtime_ready = bool(
        ...
        and runtime_cycle_age_secs is not None
        and runtime_cycle_age_secs <= 30.0
    )
    ...
    feature_data_fresh = bool(
        feature_online_ready and not bool(state.get("feature_serving_stale", False))
    )
    startup_inference_by_pair = dict(
        state.get("startup_inference_by_pair") or state.get("startup_inference") or {}
    )
    feature_serving_by_pair = dict(
        state.get("feature_serving_by_pair") or state.get("featureServingByPair") or {}
    )
    pair_readiness = dict(
        state.get("pair_readiness") or state.get("pairReadiness") or {}
    )
    strategy_engine_mode = str(
        state.get("strategy_engine_mode")
        or state.get("strategyEngineMode")
        or "supervised_legacy"
    )
    supervised_fallback = dict(
        state.get("supervised_fallback") or state.get("supervisedFallback") or {}
    )
    entry_execution_policy = dict(
        state.get("entry_execution_policy") or state.get("entryExecutionPolicy") or {}
    )
    rl_portfolio_proposal = dict(
        state.get("rl_portfolio_proposal") or state.get("rlPortfolioProposal") or {}
    )
    rl_execution_policy = dict(
        state.get("rl_execution_policy") or state.get("rlExecutionPolicy") or {}
    )
    rl_lifecycle_summary = dict(
        state.get("rl_lifecycle_summary") or state.get("rlLifecycleSummary") or {}
    )
    rl_rebalance_summary = dict(
        state.get("rl_rebalance_summary") or state.get("rlRebalanceSummary") or {}
    )
    rl_flip_intent = dict(
        state.get("rl_flip_intent") or state.get("rlFlipIntent") or {}
    )
    rl_artifact_readiness = dict(
        state.get("rl_artifact_readiness") or state.get("rlArtifactReadiness") or {}
    )
    feature_push_backlog_ok = bool(
        not bool(settings.feature_push_enabled)
        or feature_push_backlog <= int(settings.feature_push_backlog_warn)
    )
    feature_parity_ok = bool(feature_parity_breaches == 0)
    rollout_policy = dict(
        dict(state.get("runtime_diag") or {}).get("rollout_policy") or {}
    )
    rollout_runtime = dict(
        dict(dict(state.get("runtime_diag") or {}).get("risk_cycle_summary") or {}).get(
            "rollout"
        )
        or {}
    )
    ...
    active_release = (
        service.get_active_model_set(live_pair_key) if live_pair_key else {}
    )
    ...
    return {
        ...
        "database_status": str(
            health.get("database") or ("up" if database_ok else "degraded")
        ),
        "runtime_status": runtime_status,
        "runtime_ready": runtime_ready,
        ...
        "runtime_startup_warning_count": int(
            state.get("runtime_startup_warning_count") or 0
        ),
        "runtime_model_load": dict(state.get("runtime_model_load") or {}),
        "runtime_model_load_failures": int(
            state.get("runtime_model_load_failures") or 0
        ),
        "runtime_model_load_failed_pairs": list(
            state.get("runtime_model_load_failed_pairs") or []
        ),
        "runtime_model_load_degraded_pairs": list(
            state.get("runtime_model_load_degraded_pairs") or []
        ),
        ...
        "rl_supervised_fallback_used": bool(
            state.get("rl_supervised_fallback_used", False)
        ),
        "rlSupervisedFallbackUsed": bool(
            state.get("rl_supervised_fallback_used", False)
        ),
        ...
        "feature_serving_cache_hit": bool(
            state.get("feature_serving_cache_hit", False)
        ),
        "feature_serving_stale": bool(state.get("feature_serving_stale", False)),
        "feature_serving_feature_service": str(
            state.get("feature_serving_feature_service") or ""
        ),
        "feature_online_ready": feature_online_ready,
        "feature_data_fresh": feature_data_fresh,
        "feature_bar_status": str(
            state.get("feature_bar_status")
            or feature_observability.get("feature_bar_status")
            or ""
        ),
        "featureBarStatus": str(
            state.get("featureBarStatus")
            or feature_observability.get("feature_bar_status")
            or ""
        ),
        "feature_push_backlog_ok": feature_push_backlog_ok,
        ...
        "feature_push_backlog_warn": int(
            feature_observability.get("feature_push_backlog_warn") or 0
        ),
        "feature_push_backlog_overage": int(
            feature_observability.get("feature_push_backlog_overage") or 0
        ),
        "feature_blocker_reason": str(
            feature_observability.get("feature_blocker_reason") or ""
        ),
        "feature_blocker_reasons": list(
            feature_observability.get("feature_blocker_reasons") or []
        ),
        "feature_blocker_source": str(
            feature_observability.get("feature_blocker_source") or ""
        ),
        ...
        "providerRoles": dict(
            provider_health.get("roles") or state.get("provider_roles") or {}
        ),
        "provider_roles": dict(
            provider_health.get("roles") or state.get("provider_roles") or {}
        ),
...
def _state_patch_from_heartbeat_text(msg: str) -> dict[str, Any]:
    ...
    for tok in str(msg).split():
        if tok.startswith("eq="):
            ...
            patch["broker_account_scope_schema"] = str(tok.split("=", 1)[1]).strip()[
                :96
            ]
        elif tok.startswith(
            ("account_scope_version=", "broker_account_scope_version=")
        ):
            ...
            patch["broker_account_currency"] = (
                str(tok.split("=", 1)[1]).strip().upper()[:16]
            )
            ...
            patch["bridge_producer_instance_id"] = str(tok.split("=", 1)[1]).strip()[
                :128
            ]
        elif tok.startswith("terminal_lease_scope="):
            patch["bridge_terminal_lease_scope"] = str(tok.split("=", 1)[1]).strip()[
                :128
            ]
...
def _state_patch_from_report_json(payload: dict[str, Any]) -> dict[str, Any]:
            ...
            patch.update(
                {
                    ...
                    "positions_snapshot_account_scope_version": max(
                        0,
                        _safe_report_int(p.get("broker_account_scope_version"), 0),
                    ),
    ...
    if is_heartbeat and p.get("broker_account_scope") is not None:
        patch["broker_account_scope"] = str(
            p.get("broker_account_scope") or ""
        ).strip()[:128]
    ...
    if is_heartbeat and p.get("broker_account_currency") is not None:
        patch["broker_account_currency"] = (
            str(p.get("broker_account_currency") or "").strip().upper()[:16]
        )
    ...
    if report_type == "symbol_specs" and isinstance(p.get("specs"), dict):
        ...
        for sym, raw in dict(p.get("specs") or {}).items():
            ...
            if raw.get("trade_allowed") is not None:
                item["trade_allowed"] = _safe_report_bool(raw.get("trade_allowed"))
            if item:
                if broker_symbol:
    ...
    if report_type == "bridge_status" and isinstance(p.get("configured_pairs"), list):
        patch["configured_pairs"] = [
            str(x).strip().upper()
            for x in list(p.get("configured_pairs") or [])
            if str(x).strip()
        ]
    if report_type == "bridge_status" and isinstance(p.get("symbol_readiness"), dict):
        ...
        patch["symbol_ready_count"] = int(
            sum(1 for item in readiness.values() if bool(item.get("supported")))
        )
        patch["unsupported_pairs"] = sorted(
            [
                pair
                for pair, item in readiness.items()
...
def _aggregate_bars(symbol: str, timeframe: str, limit: int) -> list[dict[str, Any]]:
    ...
    source_fields = expected_source.to_fields() if expected_source is not None else {}
...
def _resolve_repo_path(raw: str) -> Path | None:
    ...
    for value in variants:
        ...
        for candidate in (
            ...
            settings.project_root.parent / path,
        ):
            if candidate.exists():
...
def _registry_candidate_rank(
    path: Path | None, *, registry_meta: dict[str, Any] | None = None
) -> float:
    if path is None:
        return 0.0
...
def _load_registry_report_details(
    *,
    ...
    if not promotion and str(registry_meta.get("promotion_status") or "").strip():
        promotion = {
            "status": str(registry_meta.get("promotion_status") or "").strip().lower()
        }
...
def _table_rows_with_count(
    ...
    *,
    ...
    try:
        ...
        rows_stmt = select(
            table,
            total_stmt.where(*clauses).scalar_subquery().label("_total_count"),
        )
        if clauses:
        ...
        rows_stmt = rows_stmt.limit(max(1, min(int(limit), 5000)))
        with engine.connect() as conn:
            raw_rows = conn.execute(rows_stmt).mappings().all()
        if not raw_rows:
            return [], 0
        total = int(raw_rows[0].get("_total_count", 0) or 0)
...
def _latest_table_row_with_count(table) -> tuple[dict[str, Any], int]:
    rows, total = _table_rows_with_count(table, limit=1)
    return (rows[0] if rows else {}), total


def _partitioned_table_rows(
    table,
    *,
    partition_column,
    ...
    keys = sorted({str(value) for value in partition_values if str(value)})
    buckets: dict[str, list[dict[str, Any]]] = {key: [] for key in keys}
    store = getattr(service, "store", None)
    engine = getattr(store, "engine", None)
    if engine is None or table is None or not keys:
        return buckets
    try:
        bounded_limit = max(1, min(int(limit_per_partition), 5000))
        ranked_stmt = select(
            table,
            partition_column.label("_partition_key"),
            func.row_number()
            .over(
        ...
        ).where(partition_column.in_(keys))
        for clause in list(where or []):
            ranked_stmt = ranked_stmt.where(clause)
        ranked = ranked_stmt.subquery()
        stmt = (
            select(ranked)
            .where(ranked.c._partition_rank <= bounded_limit)
            .order_by(ranked.c._partition_key, ranked.c._partition_rank)
        )
        with engine.connect() as conn:
            raw_rows = conn.execute(stmt).mappings().all()
        for raw_row in raw_rows:
            row = dict(raw_row)
            key = str(row.pop("_partition_key", "") or "")
            row.pop("_partition_rank", None)
...
def _orchestration_evidence_summary() -> dict[str, Any]:
    ...
    latest_experiment, experiment_count = _latest_table_row_with_count(
        store.experiment_proposals
    )
    latest_promotion, promotion_count = _latest_table_row_with_count(
        store.experiment_promotions
    )
    latest_approval, approval_event_count = _latest_table_row_with_count(
        store.approval_events
    )
    latest_experiment_payload = (
        _experiment_proposal_record(
            latest_experiment,
            approval_records=[_approval_event_record(latest_approval)]
            if latest_approval
            else [],
            promotions=[_experiment_promotion_record(latest_promotion)]
            if latest_promotion
            else [],
        )
    ...
    latest_lineage.update(
        {
            "latest_promotion_status": str(
                latest_promotion.get("status")
                or latest_lineage.get("latest_promotion_status")
                or ""
            ),
            ...
            "latest_promotion_id": str(
                latest_promotion.get("promotion_id")
                or latest_lineage.get("latest_promotion_id")
                or ""
            ),
        }
    )
...
def _workflow_lineage_summary(
    *,
    ...
    activation_package = _json_dict(
        active_caps.get("activation_package") or registry_meta.get("activation_package")
    )
    ...
    lineage.update(
        {
            ...
            "approval_status": str(
                ...
                or _json_dict(active_caps.get("promotion_summary")).get(
                    "approval_status"
                )
            ...
            "approval_record_count": int(
                len(_json_list(promotion.get("approval_records")))
            ),
...
def _caps_from_registry_meta(registry_meta: dict[str, Any]) -> dict[str, Any]:
    ...
    has_exit_model = bool(capabilities.get("has_exit_model")) or bool(
        artifacts.get("exit_policy")
    )
    ...
    warnings = list(
        registry_meta.get("activation_warnings", [])
        or registry_meta.get("warnings", [])
        or []
    )
    return {
        ...
        if (has_exit_model or has_reversal_models)
        else "runtime_soft",
        "warnings": warnings,
...
def _derive_training_workflow_status(
    *,
    ...
    promotion_status = (
        str(promotion.get("status") or registry_meta.get("promotion_status") or "")
        .strip()
        .lower()
    )
    if promotion_status:
...
def _active_lifecycle_capabilities() -> dict[str, dict[str, Any]]:
    ...
    for pair, row in service.get_active_model_sets(enabled_only=True).items():
        ...
        component_feature_services = {
            str(name): {
                "feature_service_name": str(
                    (value or {}).get("feature_service_name") or ""
                ),
                "feature_service_version": str(
                    (value or {}).get("feature_service_version") or ""
                ),
                "feature_contract_hash": str(
                    (value or {}).get("feature_contract_hash") or ""
                ),
                "feature_view_names": list(
                    (value or {}).get("feature_view_names") or []
                ),
            }
        ...
        component_feature_services.update(
            {
                ...
                for name, value in dict(
                    metadata.get("component_feature_contracts") or {}
                ).items()
                if str(name).strip()
                and str((value or {}).get("feature_service_name") or "").strip()
            }
        )
        ...
        component_versions = {
            ...
            for name, value in artifacts.items()
            if isinstance(value, dict)
            and (value.get("model_version") or value.get("model_uri"))
        }
        component_model_uris = {
            str(name): str((value or {}).get("model_uri") or "")
            for name, value in artifacts.items()
            if isinstance(value, dict)
            and str((value or {}).get("model_uri") or "").strip()
        }
        out[str(pair).upper()] = {
            "has_exit_model": bool(capabilities.get("has_exit_model"))
            or bool(artifacts.get("exit_policy")),
            "has_reversal_models": bool(capabilities.get("has_reversal_models"))
            or bool(
                artifacts.get("reversal_failure")
                and artifacts.get("reversal_opportunity")
            ),
            "activation_mode": "model_driven"
            if (
                bool(capabilities.get("has_exit_model"))
                or bool(artifacts.get("exit_policy"))
            ...
            "lineage": _json_dict(
                metadata.get("lineage") or metadata.get("lineage_snapshot")
            ),
            ...
            "activation_alias": str(
                _json_dict(metadata.get("mlflow")).get("activated_alias")
                or metadata.get("intended_alias")
                or ""
            ),
            "phase3_execution_required": bool(
                metadata.get("phase3_execution_required", False)
            ),
            ...
            "phase4_sequence_dataset_manifests": dict(
                metadata.get("phase4_sequence_dataset_manifests") or {}
            ),
            "phase4_portfolio_reports": dict(
                metadata.get("phase4_portfolio_reports") or {}
            ),
            "phase4_challenger_reports": dict(
                metadata.get("phase4_challenger_reports") or {}
            ),
            ...
            "shadow_acceptance_summary": dict(
                metadata.get("shadow_acceptance_summary") or {}
            ),
...
def _compute_workflow_status() -> dict[str, Any]:
    ...
    for pair in pairs:
        ...
        shadow_registry_file = shadow_registry_files.get(
            pair
        ) or _latest_shadow_registry_file_for_pair(pair)
        ...
        registry_file = (
            max(registry_candidates, key=lambda path: _registry_candidate_rank(path))
        ...
        if registry_file is not None:
            ...
            updated_at_ts = max(
                _registry_candidate_rank(registry_file, registry_meta=registry_meta),
                latest_report_ts,
            )
        ...
        caps = {
            **active_caps,
            **registry_caps,
            "has_exit_model": bool(active_caps.get("has_exit_model"))
            or bool(registry_caps.get("has_exit_model")),
            "has_reversal_models": bool(active_caps.get("has_reversal_models"))
            or bool(registry_caps.get("has_reversal_models")),
        ...
        selected_source = (
            "shadow"
            if (
                registry_file is not None
                and shadow_registry_file is not None
                and registry_file.resolve() == shadow_registry_file.resolve()
            )
            else "live"
        )
        ...
        workflows.append(
            {
                ...
                "startup_inference_ok": bool(
                    (startup_inference.get(pair) or {}).get("ok", False)
                ),
                "broker_symbol_ready": bool(
                    (symbol_readiness.get(pair) or {}).get("supported", False)
                ),
                "details_json": {
                    "promotion": promotion,
                    ...
                    "mlflow": _json_dict(
                        active_caps.get("mlflow") or registry_meta.get("mlflow")
                    ),
                    "component_versions": dict(
                        active_caps.get("component_versions")
                        or _json_dict(registry_meta.get("mlflow")).get(
                            "component_versions"
                        )
                        or {}
                    ),
                    "component_model_uris": dict(
                        active_caps.get("component_model_uris") or {}
                    ),
                    "component_feature_services": dict(
                        active_caps.get("component_feature_services") or {}
                    ),
                    "active_feature_services": list(
                        active_caps.get("active_feature_services") or []
                    ),
                    "activation_alias": str(
                        active_caps.get("activation_alias")
                        or registry_meta.get("intended_alias")
                        or ""
                    ),
                    ...
                    "phase3_execution_required": bool(
                        active_caps.get(
                            "phase3_execution_required",
                            registry_meta.get("phase3_execution_required", False),
                        )
                    ),
                    "phase3_evidence": dict(
                        active_caps.get("phase3_evidence")
                        or registry_meta.get("phase3_evidence")
                        or {}
                    ),
                    "phase4_shadow_only": bool(
                        active_caps.get(
                            "phase4_shadow_only",
                            registry_meta.get("phase4_shadow_only", False),
                        )
                    ),
                    "phase4_sequence_dataset_manifests": dict(
                    ...
                    "phase5_gates": dict(
                        active_caps.get("phase5_gates")
                        or registry_meta.get("phase5_gates")
                        or {}
                    ),
                    "phase5_gate_bundle": dict(
                        active_caps.get("phase5_gate_bundle")
                        or registry_meta.get("phase5_gate_bundle")
                        or {}
                    ),
                    "release_status": str(
                        active_caps.get("release_status")
                        or registry_meta.get("release_status")
                        or ""
                    ),
                    "rollback_target": dict(
                        active_caps.get("rollback_target")
                        or registry_meta.get("rollback_target")
                        or {}
                    ),
                    "operator_signoff": dict(
                        active_caps.get("operator_signoff")
                        or registry_meta.get("operator_signoff")
                        or {}
                    ),
                    "canary_plan": dict(
                        active_caps.get("canary_plan")
                        or registry_meta.get("canary_plan")
                        or {}
                    ),
                    "canary_prep": dict(
                        active_caps.get("canary_prep")
                        or registry_meta.get("canary_prep")
                        or {}
                    ),
                    "promotion_gates": list(
                        active_caps.get("promotion_gates")
                        or registry_meta.get("promotion_gates")
                        or []
                    ),
                    ...
                    "release_notes": list(
                        active_caps.get("release_notes")
                        or registry_meta.get("release_notes")
                        or []
                    ),
                    "activation_package": dict(
                        active_caps.get("activation_package")
                        or registry_meta.get("activation_package")
                        or {}
                    ),
                    ...
                    "portfolio_intelligence": dict(
                        state.get("portfolio_intelligence") or {}
                    ),
                    "capital_governance": dict(state.get("capital_governance") or {}),
                    "challenger_components": challenger_components,
                    "feature_serving": feature_serving_pair,
                    "feature_serving_source": str(
                        feature_serving_pair.get("source")
                        or state.get("feature_serving_source")
                        or ""
                    ),
                    "feature_serving_reason": str(
                        feature_serving_pair.get("reason")
                        or state.get("feature_serving_reason")
                        or ""
                    ),
                    "feature_serving_feature_service": str(
                        feature_serving_pair.get("feature_service")
                        or state.get("feature_serving_feature_service")
                        or ""
                    ),
                    "startup_inference": dict(startup_inference.get(pair) or {}),
                    "pair_readiness": dict(
                        (state.get("pair_readiness") or {}).get(pair) or {}
                    ),
...
async def v2_get_direct_mt4_bar_coverage(
    ...
    latest_direct_time = _iso(float(ordered_buckets[-1])) if ordered_buckets else None
    ...
    return {
        ...
        "ready": (
            contiguous_direct_row_count >= int(minimum) or full_snapshot_attested
        ),
    }
...
def _stage_market_bar_group(
    ...
    *,
    ...
    for item in batch.bars:
        ...
        bid_open = (
            float(payload["bid_open"]) if has_explicit_bid else mid_open - half_spread
        )
        bid_high = (
            float(payload["bid_high"]) if has_explicit_bid else mid_high - half_spread
        )
        bid_low = (
            float(payload["bid_low"]) if has_explicit_bid else mid_low - half_spread
        )
        bid_close = (
            float(payload["bid_close"]) if has_explicit_bid else mid_close - half_spread
        )
...
def _claim_bridge_command_channel(
    *,
    ...
    if (
        not any(expected)
        and str(settings.start_profile or "").strip().lower() != "live"
    ):
    ...
    if result.get("ok") is not True:
        return {
            "status": "error",
            "error": str(result.get("reason") or "bridge_consumer_lease_rejected"),
        }, 409
    return result, 200
...
async def v2_commands_events(
    limit: int = Query(500), command_id: str | None = Query(default=None)
) -> dict[str, Any]:
...
async def v2_ack_command(ack: CommandAckRequest) -> JSONResponse:
    ...
    lease, lease_code = _claim_bridge_command_channel(
        ...
        bridge_protocol_version=str(payload.get("bridge_protocol_version", "") or ""),
        channel="ack",
    )
    if lease_code != 200:
        return JSONResponse(content=lease, status_code=lease_code)
...
def _prepare_market_tick_payload(
    payload: dict[str, Any],
    *,
    market_source: AuthenticatedMarketSource | None,
    received_at: float,
) -> tuple[dict[str, Any], dict[str, Any] | None]:
    """Normalize one validated quote without publishing side effects."""
    ...
    return tick, persistence_payload


def _apply_prepared_market_tick(
    tick: dict[str, Any],
    *,
    market_event_persisted: bool,
) -> None:
    sym = str(tick.get("symbol") or "").strip().upper()
    if market_event_persisted:
...
def _ingest_market_tick_payload(
    ...
    *,
    ...
    """Persist and publish one validated quote."""

    tick, persistence_payload = _prepare_market_tick_payload(
        payload,
        market_source=market_source,
        received_at=received_at,
    )
    if persistence_payload is not None:
        service.record_tick(persistence_payload)
    _apply_prepared_market_tick(
        tick,
        market_event_persisted=persistence_payload is not None,
    )
    return persistence_payload is not None
...
async def v2_ticks(batch: MarketTickBatchRequest) -> dict[str, Any]:
    ...
    if (
        ...
        and market_source.producer_identity == _BAR_SEED_RECOVERY_PRODUCER_IDENTITY
        and not any(
    ...
    received_at = _utc_now_ts()
    source_payload = {
        key: value for key, value in batch_payload.items() if key != "ticks"
    }
    prepared_ticks: list[tuple[dict[str, Any], dict[str, Any] | None]] = []
    for item in batch.ticks:
        prepared_ticks.append(
            _prepare_market_tick_payload(
                {
    ...
    persistence_payloads = [
        persistence_payload
        for _tick, persistence_payload in prepared_ticks
        if persistence_payload is not None
    ]
    if persistence_payloads:
        service.record_ticks(persistence_payloads)
    for tick, persistence_payload in prepared_ticks:
        _apply_prepared_market_tick(
            tick,
            market_event_persisted=persistence_payload is not None,
        )
    return {
        ...
        "persisted": len(persistence_payloads),
    }
...
async def v2_reports_post(request: Request) -> dict[str, Any]:
    ...
    if parsed is not None:
        ...
        text = json.dumps(
            parsed, separators=(",", ":"), sort_keys=True, allow_nan=False
        )
    ...
    if isinstance(parsed, dict) and report_type == "heartbeat":
        ...
        heartbeat_lease, heartbeat_lease_code = _claim_bridge_command_channel(
            ...
            bridge_protocol_version=str(parsed.get("bridge_protocol_version") or ""),
            channel="heartbeat",
        )
        ...
        parsed is None
        and text.upper().startswith("HEARTBEAT")
        and _market_source_contract_required()
    ):
        raise _market_source_ingest_error(
    ...
    if isinstance(parsed, dict) and authenticated_report_source is not None:
        ...
        prior_source_id = str(
            dict(prior_state.get("bridge_market_source") or {}).get("market_source_id")
            or ""
        )
        source_fields = authenticated_report_source.to_fields()
        if (
            report_type == "heartbeat"
            and prior_source_id != authenticated_report_source.source_id
        ):
...
async def v2_decision_snapshots_get(limit: int = Query(200)) -> dict[str, Any]:
    return {
        "items": service.get_decision_snapshots(limit=max(1, min(int(limit), 5000)))
    }
...
async def v2_orchestration_experiments_get(limit: int = Query(200)) -> dict[str, Any]:
    ...
    store = getattr(service, "store", None)
    experiment_table = getattr(store, "experiment_proposals", None)
    promotion_table = getattr(store, "experiment_promotions", None)
    approval_table = getattr(store, "approval_events", None)
    experiment_rows, experiment_count = _table_rows_with_count(
        experiment_table, limit=lim
    )
    promotion_rows, promotion_count = _table_rows_with_count(
        promotion_table, limit=lim
    )
    approval_rows, approval_event_count = _table_rows_with_count(
        approval_table, limit=lim
    )
    experiment_ids = [
    ...
    approvals_by_experiment = (
        _partitioned_table_rows(
            approval_table,
            partition_column=approval_table.c.subject_id,
            partition_values=experiment_ids,
            limit_per_partition=lim,
            where=[
                approval_table.c.subject_type.in_(
    ...
    promotions_by_experiment = (
        _partitioned_table_rows(
            promotion_table,
            partition_column=promotion_table.c.experiment_id,
            partition_values=experiment_ids,
            limit_per_partition=lim,
        )
    ...
    for row in experiment_rows:
        ...
        items.append(
            _experiment_proposal_record(
                row,
                approval_records=[
                    _approval_event_record(item)
                    for item in approvals_by_experiment.get(experiment_id, [])
                ]
                if experiment_id
                else [],
                promotions=[
                    _experiment_promotion_record(item)
                    for item in promotions_by_experiment.get(experiment_id, [])
                ]
                if experiment_id
                else [],
            )
    ...
    for item in items:
        experiment_status_counts[
            str(item.get("approval_status") or "").strip().lower()
        ] += 1
    ...
    return {
        ...
        "summary": {
            ...
            "promotion_count": promotion_count,
            "approval_event_count": approval_event_count,
            "latest_experiment_id": str(items[0].get("experiment_id") or "")
            if items
            else "",
            "latest_promotion_id": str(promotion_rows[0].get("promotion_id") or "")
            if promotion_rows
            else "",
            "latest_approval_event_id": str(approval_rows[0].get("event_id") or "")
            if approval_rows
            else "",
            "latest_approval_decision": str(approval_rows[0].get("decision") or "")
            if approval_rows
            else "",
            "latest_lineage": latest_lineage,
            "approval_status_counts": {
                k: v for k, v in experiment_status_counts.items() if k
            },
            "promotion_status_counts": {
                k: v for k, v in promotion_status_counts.items() if k
            },
            "approval_decision_counts": {
                k: v for k, v in approval_decision_counts.items() if k
            },
        },
    }
...
async def v2_orchestration_experiment_get(
    experiment_id: str, limit: int = Query(200)
) -> dict[str, Any]:
    store = getattr(service, "store", None)
    ...
    experiment_rows, experiment_count = _table_rows_with_count(
        table, limit=1, where=[table.c.experiment_id == str(experiment_id)]
    )
    if not experiment_rows:
        raise HTTPException(status_code=404, detail="experiment not found")
    approval_table = getattr(store, "approval_events", None)
    promotion_table = getattr(store, "experiment_promotions", None)
    approvals, approval_event_count = _table_rows_with_count(
        approval_table,
        limit=limit,
        where=[
            approval_table.c.subject_type.in_(["experiment", "experiment_proposal"]),
            approval_table.c.subject_id == str(experiment_id),
        ],
    )
    promotions, promotion_count = _table_rows_with_count(
        promotion_table,
        limit=limit,
        where=[promotion_table.c.experiment_id == str(experiment_id)],
    )
...
async def v2_orchestration_promotions_get(limit: int = Query(200)) -> dict[str, Any]:
    ...
    store = getattr(service, "store", None)
    promotion_rows, promotion_count = _table_rows_with_count(
        getattr(store, "experiment_promotions", None), limit=lim
    )
    approval_rows, approval_event_count = _table_rows_with_count(
        getattr(store, "approval_events", None), limit=lim
    )
    ...
    return {
        ...
        "summary": {
            "promotion_count": promotion_count,
            "approval_event_count": approval_event_count,
            "latest_promotion_id": str(items[0].get("promotion_id") or "")
            if items
            else "",
            "latest_experiment_id": str(items[0].get("experiment_id") or "")
            if items
            else "",
            "latest_promotion_status": str(items[0].get("status") or "")
            if items
            else "",
            "promotion_status_counts": {
                k: v for k, v in promotion_status_counts.items() if k
...
async def v2_closed_trades_get(limit: int = Query(200)) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        trades.append(trade)
    trades.sort(
        key=lambda item: float(item.get("close_time_epoch") or 0.0), reverse=True
    )
    ...
    governance_events = [
        {
            ...
            "status": (
                "error"
                if str(item.get("event_type") or "").strip().lower()
                in {
                    "runtime_startup_failed",
                    "feature_push_failed",
                    "feature_parity_breach",
                }
                else "warning"
            ),
...
async def v2_state() -> dict[str, Any]:
    raw_state, raw_metrics = service.get_state_and_metrics()
    metrics = dict(raw_metrics or {})
    state = _state_with_liveness(
        raw_state,
        include_decision_snapshot=True,
        metrics=metrics,
    )
    health = dict(service.get_health() or {})
    database_ok = bool(health.get("tables_ok"))
    state["database_ok"] = database_ok
    state["database_status"] = str(
        health.get("database") or ("up" if database_ok else "degraded")
    )
    if not database_ok:
    ...
    orchestration_live = _orchestration_live_summary(
        ...
        active_release=service.get_active_model_set(live_pair_key)
        if live_pair_key
        else {},
        execution_uncertainty=_execution_uncertainty_for_readiness(),
    )
...
async def v2_metrics() -> dict[str, Any]:
    raw_state, raw_metrics = service.get_state_and_metrics()
    out = dict(raw_metrics or {})
    state = _state_with_liveness(raw_state, metrics=out)
    ...
    out["feature_serving_cache_hit"] = bool(
        state.get("feature_serving_cache_hit", False)
    )
    out["feature_serving_stale"] = bool(state.get("feature_serving_stale", False))
    out["feature_serving_feature_service"] = str(
        state.get("feature_serving_feature_service") or ""
    )
    out["feature_online_ready"] = bool(
        str(state.get("feature_serving_source") or "").strip()
    )
    out["feature_data_fresh"] = bool(
        out["feature_online_ready"]
        and not bool(state.get("feature_serving_stale", False))
    )
    ...
    out["feature_push_backlog_overage"] = int(
        state.get("feature_push_backlog_overage") or 0
    )
    ...
    out["feature_parity_ok"] = bool(
        int(dict(out.get("feature_parity") or {}).get("breaches") or 0) == 0
    )
    out["risk_cycle_summary"] = dict(
        dict(state.get("runtime_diag") or {}).get("risk_cycle_summary") or {}
    )
    out["rollout_policy"] = dict(
        dict(state.get("runtime_diag") or {}).get("rollout_policy") or {}
    )
...
async def v2_health() -> dict[str, Any]:
    raw_state, raw_metrics = service.get_state_and_metrics()
    metrics = dict(raw_metrics or {})
    state = _state_with_liveness(raw_state, metrics=metrics)
    ...
    out["entriesOnlyMode"] = bool(state.get("entries_only_mode", False))
    out["shadowOnlyMode"] = bool(state.get("shadow_only_mode", False))
    out["paper_execution"] = dict(
        state.get("paper_execution") or state.get("paperExecution") or {}
    )
...
def _reconcile_ea_positions() -> tuple[list[PositionView], float | None]:
    ...
    for row in reports:
        ...
        if (
            str(payload.get("report_type") or "").strip().lower()
            != "positions_snapshot"
        ):
            continue
...
async def v2_positions_reconcile() -> PositionReconcileResponse:
    ...
    ea_market_source_id = str(state.get("positions_snapshot_market_source_id") or "")
