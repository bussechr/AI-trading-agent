...
def run_lean_harness(
    *,
    ...
    if execute and (model_manifest_path is None or economic_report_path is None):
        raise ValueError("executed LEAN harness requires model_manifest_path and economic_report_path")
    if execute and not Path(model_manifest_path).is_file():
        raise ValueError("executed LEAN harness model manifest is missing")
    ...
    if execute:
        if output_dir.exists():
            raise ValueError("executed LEAN harness requires a fresh output_dir")
        if economic_report is None or not economic_report.is_relative_to(output_dir):
            raise ValueError("executed LEAN harness economic report must be inside output_dir")
        engine_version = _lean_engine_version()
        if not str(dataset_hash).strip():
            raise ValueError("executed LEAN harness dataset_hash is required")
        if not engine_version:
            raise ValueError("executed LEAN harness engine_version is required")
        normalized_stress_paths = {
            str(name): Path(path).resolve()
            for name, path in dict(
                stress_report_paths
                or {
                    name: output_dir / f"lean.stress.{name}.json"
                    for name in REQUIRED_EXTERNAL_STRESS_SCENARIOS
                }
            ).items()
        }
        if set(normalized_stress_paths) != set(REQUIRED_EXTERNAL_STRESS_SCENARIOS):
            raise ValueError("executed LEAN harness requires every external stress scenario")
        for name, path in normalized_stress_paths.items():
            if not path.is_relative_to(output_dir):
                raise ValueError(f"executed LEAN harness stress report must be inside output_dir:{name}")
            if path.exists():
                raise ValueError(f"executed LEAN harness stress report path already exists:{name}")
        if economic_report.exists():
            raise ValueError("executed LEAN harness economic report path already exists")
        ...
        if not identity.model_set_id or not identity.bundle_run_id or not identity.artifact_set_sha256:
            raise ValueError("executed LEAN harness model identity is incomplete")
