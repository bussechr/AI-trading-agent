...
def build_nautilus_command(*, bundle_dir: Path, output_dir: Path, extra_args: list[str] | None = None) -> list[str]:
    configured = str(os.environ.get("FXSTACK_NAUTILUS_CMD", "")).strip()
    if configured:
        base = configured.split()
    else:
        base = ["python", "-m", "nautilus_trader"]
    return [*base, "backtest", "--bundle", str(bundle_dir), "--out", str(output_dir), *(list(extra_args or []))]
...
def run_nautilus_harness(
    *,
    ...
    command = build_nautilus_command(bundle_dir=bundle_dir, output_dir=output_dir, extra_args=extra_args)
    status = "planned"
    ...
    return HarnessRunManifest(
        ...
        metadata={"execute": bool(execute), "engine_package": "nautilus_trader"},
    )
