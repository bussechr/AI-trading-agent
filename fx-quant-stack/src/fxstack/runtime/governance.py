...
from fxstack._serialization import copy_json_payload
...
class ProviderHealthSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "details": copy_json_payload(self.details),
        }
...
class CapitalGovernanceState:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "capital_band": self.capital_band,
            "mode": self.mode,
            "paused": self.paused,
            ...
            "eligible_for_upgrade": self.eligible_for_upgrade,
            "rollback_actions": [item.to_dict() for item in self.rollback_actions],
            "metrics": copy_json_payload(self.metrics),
        }
...
def capital_band_budget_scale(capital_band: str, settings: Any) -> float:
    ...
    if band == "micro_live":
        return float(
            getattr(settings, "capital_rollout_budget_scale_micro_live", 0.1) or 0.1
        )
    if band == "low_risk_live":
        return float(
            getattr(settings, "capital_rollout_budget_scale_low_risk", 0.25) or 0.25
        )
    if band == "full_risk_live":
        return float(
            getattr(settings, "capital_rollout_budget_scale_full_risk", 1.0) or 1.0
        )
...
def _feature_serving_stale_summary(
...
def compute_capital_governance_state(
    *,
    ...
    capital_band = (
        str(getattr(settings, "capital_band_mode", "paper") or "paper").strip().lower()
    )
    ...
    latency_budget = float(
        getattr(settings, "phase5_canary_latency_budget_ms", 0.0) or 0.0
    )
    stale_features, selected_pairs_count, selected_stale_count = (
        _feature_serving_stale_summary(runtime_diag)
    )
    parity_breaches = int(
        dict(metrics.get("feature_parity") or {}).get("breaches") or 0
    )
    rollout_breaches = int(
        dict(runtime_diag.get("risk_cycle_summary") or {}).get("rollout_breach_count")
        or 0
    )
    top_concentration_share = _safe_float(
        concentration.get("top_symbol_share", 0.0), 0.0
    )
    ...
    concentration_soft_limit = min(
        _CONCENTRATION_SOFT_LIMIT, max(0.0, concentration_hard_limit - 0.15)
    )
    ...
    net_exposure_share = (
        float(net_exposure / gross_exposure) if gross_exposure > 0.0 else 0.0
    )
    net_lot_exposure_share = (
        float(net_lot_exposure / gross_lot_exposure)
        if gross_lot_exposure > 0.0
        else 0.0
    )
    correlation_method = (
        str(
            budget.get("correlation_method") or correlation.get("method") or "heuristic"
        )
        .strip()
        .lower()
    )
    correlation_sample_count = int(
        budget.get("correlation_sample_count") or correlation.get("sample_count") or 0
    )
    ...
    configured_corr_soft_limit = min(
        _CORRELATION_SOFT_LIMIT, max(0.0, configured_corr_hard_limit - 0.20)
    )
    ...
    if correlation_method in {"realized", "hybrid"} and correlation_sample_count > 0:
        confidence_denominator = max(
            int(correlation_window_bars or correlation_min_obs or 8), 8
        )
        confidence = min(
            1.0, float(correlation_sample_count) / float(confidence_denominator)
        )
        correlation_strength *= confidence
    else:
        correlation_strength *= 0.85
    concentration_strength = max(
        top_concentration_share, top_currency_share, symbol_hhi, currency_hhi
    )
    ...
    concentration_stress = _safe_float(
        budget.get("concentration_stress", concentration_strength),
        concentration_strength,
    )
    currency_stress = _safe_float(
        budget.get("currency_stress", max(top_currency_share, currency_hhi)),
        max(top_currency_share, currency_hhi),
    )
    session_stress = _safe_float(
        budget.get("session_stress", max(session_peak_share, session_penalty)),
        max(session_peak_share, session_penalty),
    )
    correlation_pressure = _normalized_excess(
        correlation_strength, configured_corr_soft_limit, configured_corr_hard_limit
    )
    concentration_pressure = _normalized_excess(
        concentration_strength, concentration_soft_limit, concentration_hard_limit
    )
    exposure_pressure = _normalized_excess(
        exposure_strength, _EXPOSURE_SOFT_LIMIT, _EXPOSURE_HARD_LIMIT
    )
    ...
    tail_loss_limit_pct = max(
        0.0, _safe_float(getattr(settings, "capital_max_tail_loss_pct", 0.0), 0.0)
    )
    tail_loss_gate_mode = (
        str(getattr(settings, "capital_tail_loss_gate_mode", "warn") or "warn")
        .strip()
        .lower()
    )
    ...
    worst_case_tail_loss = abs(
        _safe_float(stress.get("worst_case_loss_proxy", 0.0), 0.0)
    )
    ...
    tail_loss_pct = (
        float(worst_case_tail_loss / current_equity * 100.0)
        if tail_loss_measured
        else 0.0
    )
    ...
    market_pressure = max(
        correlation_pressure, concentration_pressure, exposure_pressure
    )
    effective_market_pressure = market_pressure if governance_enabled else 0.0
    if governance_enabled:
        latency_breaches = (
            1 if latency_budget > 0.0 and loop_latency_ms > latency_budget else 0
        )
        if latency_breaches > int(
            getattr(settings, "capital_max_latency_breach_count", 0) or 0
        ):
            reasons.append("latency_breach")
        if stale_features > int(
            getattr(settings, "capital_max_stale_feature_count", 0) or 0
        ):
            reasons.append("stale_features")
        if parity_breaches > int(
            getattr(settings, "capital_max_operational_fault_count", 0) or 0
        ):
        ...
        if effective_market_pressure >= _MARKET_PRESSURE_ENTRY_THRESHOLD:
            if correlation_pressure > 0.0:
                reasons.append(
                    ...
                    if correlation_method in {"realized", "hybrid"}
                    else "heuristic_correlation"
                )
        ...
        if paused
            ...
            if shadow_only
                ...
                if entries_only
                    ...
                    "degraded"
                    if effective_market_pressure >= _MARKET_PRESSURE_DEGRADE_THRESHOLD
                    else "normal"
                )
            )
        )
    ...
    rollback_actions = [
        RollbackAction(
            ...
            armed=bool("stale_features" in reasons),
            reason="stale_features",
            scope="features",
        ),
        RollbackAction(
            action="model_rollback",
            armed=bool("parity_breach" in reasons),
            reason="parity_breach",
            scope="mlflow",
        ),
        RollbackAction(
            action="execution_rollback",
            armed=bool(entries_only),
            reason="entries_only",
            scope="execution",
        ),
        RollbackAction(
    ...
    return CapitalGovernanceState(
        ...
        eligible_for_upgrade=bool(
            not reasons
            and capital_band in {"micro_live", "low_risk_live", "full_risk_live"}
        ),
...
def _force_governance_pause(payload: dict[str, Any], reason: str) -> dict[str, Any]:
    ...
    rollback_actions = [
        dict(item or {}) for item in list(out.get("rollback_actions") or [])
    ]
    by_action = {str(item.get("action") or ""): item for item in rollback_actions}
    for action, scope in (
        ("execution_rollback", "execution"),
        ("global_rollback", "runtime"),
    ):
...
def compute_binding_capital_governance_snapshot(
    *,
    ...
    if previous_ts <= 0.0 or now <= 0.0:
        ...
        source_age_secs is None
        or not math.isfinite(source_age_secs)
        or source_age_secs < 0.0
        or source_age_secs > max_age
    ):
