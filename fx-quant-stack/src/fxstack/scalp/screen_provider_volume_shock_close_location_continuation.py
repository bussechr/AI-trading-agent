# AGENT: ROLE: Research-only Provider-Volume Shock Close-Location Continuation bid/ask M1 discovery screen.
# AGENT: ENTRYPOINT: `screen_universe`; CLI `python -m fxstack.scalp.screen_provider_volume_shock_close_location_continuation`.
# AGENT: PRIMARY INPUTS: immutable dual-volume bid/ask M1, proxy costs, and acquisition provenance.
...
"""Causal M1 Provider-Volume Shock Close-Location Continuation (PVSCLC) screen.

For a closed signal bar ``t``, both baselines contain exactly 240 consecutive
completed bars from ``t-240`` through ``t-1``.  ``V90`` is the Hyndman-Fan
type-7 90th percentile of ``bid_volume + ask_volume`` and ``S25`` is the same
type-7 25th percentile of close spread.  Provider volume is an activity proxy,
not signed order flow.

The signal requires activity strictly above a positive ``V90``.  BUY requires
positive bid and ask log bodies of at least ``C=P+1bp`` and both closes in the
top 20% of their nonzero quote ranges.  SELL is the exact sign/location mirror.
Signal-close and exact ``t+1`` entry-open spreads cannot exceed
``min(S25, P)``.
...
IMMUTABLE_PRIOR_ATTEMPTED_CELLS = 4_422
IMMUTABLE_CURRENT_ATTEMPTED_CELLS = 36
IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS = 4_458
TWO_SIDED_BONFERRONI_FAMILY_ALPHA = 0.05
BONFERRONI_STUDENT_T_MIN_DF = 99
TWO_SIDED_BONFERRONI_STUDENT_T_MIN_DF99_ABS_THRESHOLD = 4.627867074083506
PROXY_CONTRACT_SCHEMA_VERSION = "fxstack.scalp.pair_proxy_spread_contract.v1"
ACTIVITY_PROVENANCE_SCHEMA_VERSION = (
    "fxstack.scalp.dual_side_volume_snapshot_provenance.v1"
)
MINIMUM_SOURCE_ROWS_PER_SYMBOL = 175_000
MAXIMUM_BOUNDARY_LAG_SECONDS = 604_800
MAXIMUM_SOURCE_GAP_SECONDS = 345_600
TIMESTAMP_SET_HASH_GRAMMAR = "sha256_ascii_lf_header_timestamp_then_canonical_rows_v1"
ACTIVITY_PROVENANCE_TOOL_FIELDS = frozenset(
    {
        "tool_path",
        "tool_sha256",
...
ACTIVITY_PROVENANCE_SYMBOL_FIELDS = frozenset(
    {
        "instrument_id",
        "output_file",
        "output_sha256",
...
ACTIVITY_PROVENANCE_VOLUME_COUNT_FIELDS = frozenset(
    {"rows", "finite", "nonnegative", "missing", "zero_count"}
)
SOURCE_ACTIVITY_INTEGRITY_FIELDS = frozenset(
    {
        "rows",
        "bid_stream_sha256",
        "ask_stream_sha256",
...
class PVSCLCConfig:
    """The single frozen PVSCLC configuration."""
    ...
    body_recorded_cost_multiple: float = BODY_RECORDED_COST_MULTIPLE
...
GRID: tuple[PVSCLCConfig, ...] = (PVSCLCConfig(),)
PVSCLC_CONFIG_BY_ID: dict[str, PVSCLCConfig] = {
    config.config_id: config for config in GRID
}
...
class PVSCLCClosedSignal:
    ...
    signal_bid_volume: float
    signal_ask_volume: float
    spread_q25_bps: float
    proxy_budget_bps: float
    spread_cap_bps: float
    bid_log_body_bps: float
    ask_log_body_bps: float
    bid_close_location: float
    ask_close_location: float
...
PVSCLC_SIGNAL_FIELD_NAMES = frozenset(PVSCLCSignal.__dataclass_fields__)
PVSCLC_TRADE_FIELD_NAMES = frozenset(PVSCLCTrade.__dataclass_fields__)
PVSCLC_RESERVATION_OUTCOME_FIELD_NAMES = frozenset(
    {
        "event_id",
        "reservation_status",
        "outcome_reason",
...
PVSCLC_RESERVATION_FIELD_NAMES = (
    PVSCLC_SIGNAL_FIELD_NAMES | PVSCLC_RESERVATION_OUTCOME_FIELD_NAMES
)
PVSCLC_MISSING_FILL_RESERVATION_FIELD_NAMES = PVSCLC_RESERVATION_FIELD_NAMES | {
    "gate_risk_basis_bps"
}
...
class PreparedSeries:
    bars: list[QuoteBar]
    activity_v90_before_signal: list[float | None]
...
def validate_quote_bar(bar: QuoteBar) -> bool:
    ...
    if bar.epoch % 60 != 0 or not all(
        ...
        return False
    if (
        not math.isfinite(bar.bid_volume)
        or bar.bid_volume < 0.0
        or not math.isfinite(bar.ask_volume)
        or bar.ask_volume < 0.0
        or not math.isfinite(bar.activity)
    ):
        return False
    if not (
...
def _parse_epoch(text: str | None) -> int | None:
    ...
    main = (
        raw[:-1] if raw.endswith("Z") else raw[:-6] if raw.endswith("+00:00") else raw
    )
    if "." in main and main.rsplit(".", 1)[1] != "000000":
        raise ValueError("timestamp fractional seconds must be absent or exactly zero")
    try:
...
def _load_m1_csv_bytes(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"{source_name}: M1 CSV must be UTF-8") from exc
    rows: list[QuoteBar] = []
    last_epoch: int | None = None
    reader = csv.reader(io.StringIO(text, newline=""))
    if tuple(next(reader, None) or ()) != CSV_HEADER:
        raise ValueError(f"{source_name}: unexpected M1 header")
    for row_number, row in enumerate(reader, start=2):
        if len(row) != len(CSV_HEADER):
            raise ValueError(f"{source_name}:{row_number}: malformed M1 row")
        try:
            epoch = _parse_epoch(row[0])
            assert epoch is not None
            values = [float(value) for value in row[1:]]
        except (AssertionError, TypeError, ValueError) as exc:
            raise ValueError(f"{source_name}:{row_number}: malformed M1 row") from exc
        if last_epoch is not None and epoch <= last_epoch:
            raise ValueError(f"{source_name}:{row_number}: non-monotonic timestamp")
        last_epoch = epoch
        bar = QuoteBar(epoch, *values)
        if not validate_quote_bar(bar):
            raise ValueError(f"{source_name}:{row_number}: invalid bid/ask OHLC")
        if start_epoch is not None and epoch < start_epoch:
            if reject_outside_window:
                raise ValueError(
                    f"{source_name}:{row_number}: row before evaluation window"
                )
            continue
        if end_epoch is not None and epoch >= end_epoch:
            if reject_outside_window:
                raise ValueError(
                    f"{source_name}:{row_number}: row at or after evaluation window"
                )
            continue
        rows.append(bar)
    return rows
...
def load_m1_csv(
    ...
    return _load_m1_csv_bytes(
        path.read_bytes(), source_name=path.name, start=start, end=end
    )
...
def _type7_quantile(sorted_values: Sequence[float], probability: float) -> float:
    """Return the exact Hyndman-Fan type-7 sample quantile."""

    if not sorted_values or not 0.0 <= probability <= 1.0:
        raise ValueError("type-7 quantile requires data and 0 <= p <= 1")
...
def _build_pre_signal_context(
    ...
    """Build type-7 V90/S25 over exactly t-240 through t-1."""

    size = len(bars)
    activity_v90: list[float | None] = [None] * size
    spread_q25: list[float | None] = [None] * size
    for run_start, run_end in _consecutive_runs(bars):
        first_signal = run_start + BASELINE_M1_BARS
        if first_signal >= run_end:
            continue
        baseline_start = first_signal - BASELINE_M1_BARS
        baseline_end = first_signal
        spread_ordered = sorted(
            bars[index].spread_close_bps
            for index in range(baseline_start, baseline_end)
        )
        activity_ordered = sorted(
            bars[index].activity for index in range(baseline_start, baseline_end)
        )
        if (
            len(spread_ordered) != BASELINE_M1_BARS
            or len(activity_ordered) != BASELINE_M1_BARS
        ):
            raise RuntimeError("PVSCLC rolling baseline length changed")
        for signal_index in range(first_signal, run_end):
            v90 = _type7_quantile(activity_ordered, ACTIVITY_QUANTILE)
            q25 = _type7_quantile(spread_ordered, SPREAD_QUANTILE)
            if math.isfinite(v90) and v90 >= 0.0 and math.isfinite(q25) and q25 >= 0.0:
                activity_v90[signal_index] = v90
            ...
            _remove_sorted(activity_ordered, bars[expired_index].activity)
            bisect.insort(activity_ordered, bars[admitted_index].activity)
    return activity_v90, spread_q25
...
def baseline_context_at(
    ...
    if signal_index < 0 or signal_index >= len(prepared.bars):
        return None
    activity_v90 = prepared.activity_v90_before_signal[signal_index]
    q25 = prepared.spread_q25_before_signal[signal_index]
    if (
        activity_v90 is None
        or q25 is None
        or not math.isfinite(activity_v90)
        or activity_v90 < 0.0
        or not math.isfinite(q25)
        or q25 < 0.0
    ):
        return None
    return BaselineContext(float(activity_v90), float(q25))
...
def _is_canonical_utc_second(value: Any) -> bool:
    ...
    return bool(
        epoch is not None
        and value
        == dt.datetime.fromtimestamp(epoch, dt.timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
    )
...
def _normalized_p_star(
    *, recorded_cost_bps: float, execution_cost_debit_bps: float
    ...
    if execution_cost_debit_bps == recorded_cost_bps and p_star != MAX_P_STAR:
        raise RuntimeError("PVSCLC normalized p-star boundary lost exactness")
    if execution_cost_debit_bps <= recorded_cost_bps and p_star > MAX_P_STAR:
        raise RuntimeError("PVSCLC normalized p-star exceeded its derived maximum")
    return p_star
...
def evaluate_closed_signal(
    *,
...
) -> tuple[PVSCLCClosedSignal | None, str]:
    """Evaluate the closed-t PVSCLC trigger without reading or requiring t+1."""
    ...
    if context.activity_v90 <= 0.0:
        return None, "activity_v90_not_positive"
    signal_activity = signal_bar.activity
    if signal_activity <= context.activity_v90:
        return None, "activity_not_strictly_above_v90"
    ...
    bid_range = signal_bar.bid_h - signal_bar.bid_l
    ask_range = signal_bar.ask_h - signal_bar.ask_l
    if bid_range <= 0.0 or ask_range <= 0.0:
        return None, "zero_quote_range"
    bid_body = 1e4 * math.log(signal_bar.bid_c / signal_bar.bid_o)
    ask_body = 1e4 * math.log(signal_bar.ask_c / signal_bar.ask_o)
    recorded_cost = budget + FROZEN_EXTRA_ROUND_TRIP_COST_BPS
    required_body = config.body_recorded_cost_multiple * recorded_cost
    if side == "BUY":
        bid_close_location = (signal_bar.bid_c - signal_bar.bid_l) / bid_range
        ask_close_location = (signal_bar.ask_c - signal_bar.ask_l) / ask_range
        if bid_body < required_body or ask_body < required_body:
            return None, "bid_or_ask_log_body_below_1c"
    else:
        bid_close_location = (signal_bar.bid_h - signal_bar.bid_c) / bid_range
        ask_close_location = (signal_bar.ask_h - signal_bar.ask_c) / ask_range
        if -bid_body < required_body or -ask_body < required_body:
            return None, "bid_or_ask_log_body_below_1c"
    if (
        bid_close_location < config.close_location_threshold
        or ask_close_location < config.close_location_threshold
    ):
        return None, "bid_or_ask_close_location_below_0_80"
    ...
    return (
        PVSCLCClosedSignal(
            ...
            activity_v90=context.activity_v90,
            signal_activity=signal_activity,
            signal_activity_ratio=signal_activity / context.activity_v90,
            signal_bid_volume=signal_bar.bid_volume,
            signal_ask_volume=signal_bar.ask_volume,
            spread_q25_bps=context.spread_q25_bps,
            proxy_budget_bps=budget,
            spread_cap_bps=spread_cap,
            bid_log_body_bps=bid_body,
            ask_log_body_bps=ask_body,
            bid_close_location=bid_close_location,
            ask_close_location=ask_close_location,
...
def _complete_signal_at_exact_next_open(
    *, prepared: PreparedSeries, closed: PVSCLCClosedSignal
) -> tuple[PVSCLCSignal | None, str]:
    ...
    if not _p_star_forms_agree(
        ...
        raise RuntimeError("PVSCLC normalized and standard p-star forms disagree")
    ...
    return (
        PVSCLCSignal(
            ...
            activity_metric_id=closed.activity_metric_id,
            activity_v90=closed.activity_v90,
            signal_activity=closed.signal_activity,
            signal_activity_ratio=closed.signal_activity_ratio,
            signal_bid_volume=closed.signal_bid_volume,
            signal_ask_volume=closed.signal_ask_volume,
            spread_q25_bps=closed.spread_q25_bps,
            proxy_budget_bps=closed.proxy_budget_bps,
            spread_cap_bps=closed.spread_cap_bps,
            bid_log_body_bps=closed.bid_log_body_bps,
            ask_log_body_bps=closed.ask_log_body_bps,
            bid_close_location=closed.bid_close_location,
            ask_close_location=closed.ask_close_location,
...
def _event_id_fields(
    *, config_id: str, symbol: str, side: str, signal_epoch: int, entry_epoch: int
    ...
    raw = f"provider_volume_shock_close_location_continuation|{config_id}|{symbol}|{side}|{signal_epoch}|{entry_epoch}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:24]


def _event_id(signal: PVSCLCSignal) -> str:
...
def simulate_trade(
    bars: Sequence[QuoteBar], *, signal: PVSCLCSignal
) -> PVSCLCTrade | None:
    ...
    raise RuntimeError("complete PVSCLC outcome horizon did not resolve")


def _reservation_row(
    signal: PVSCLCSignal, *, status: str, trade: PVSCLCTrade | None
) -> dict[str, Any]:
    gate_pnl_r = (
        trade.pnl_r
...
def _missing_fill_reservation_row(
    closed: PVSCLCClosedSignal, *, reason: str
) -> dict[str, Any]:
    spread_stress = closed.proxy_budget_bps
    recorded_cost = spread_stress + FROZEN_EXTRA_ROUND_TRIP_COST_BPS
...
def _adverse_gap_diagnostics(trades: Sequence[PVSCLCTrade]) -> dict[str, Any]:
    gap_returns = [
...
def screen_cell(
    *,
    ...
    config: PVSCLCConfig,
    proxy_spread_budget_bps: float | None,
    evaluation_end_epoch: int | None = None,
) -> dict[str, Any]:
    """Generate the complete ordered source replay for one fixed PVSCLC cell."""

    trades: list[PVSCLCTrade] = []
    ...
    for index in range(len(prepared.bars)):
        ...
        if (
            evaluation_end_epoch is not None
            and closed.expected_entry_epoch >= evaluation_end_epoch
        ):
            reasons["entry_outside_evaluation_window"] += 1
            continue
        ...
        if signal is None:
            ...
            if reason == "entry_spread_above_q25_or_proxy":
                continue
            raise RuntimeError(f"PVSCLC completion readiness failure: {reason}")
        eligible_events += 1
        ...
        if trade is None:
            raise RuntimeError("complete PVSCLC horizon unexpectedly failed to score")
        trades.append(trade)
    ...
    return {
        ...
        **gap_diagnostics,
        ...
        "activity_metric_id": ACTIVITY_METRIC_ID,
        "activity_quantile": config.activity_quantile,
        "spread_quantile": config.spread_quantile,
        "close_location_threshold": config.close_location_threshold,
        "body_recorded_cost_multiple": config.body_recorded_cost_multiple,
...
def trial_accounting() -> dict[str, int]:
    ...
    if current != IMMUTABLE_CURRENT_ATTEMPTED_CELLS:
        raise RuntimeError("PVSCLC fixed-family accounting changed")
    if (
        IMMUTABLE_PRIOR_ATTEMPTED_CELLS + current
        != IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS
    ):
        raise RuntimeError("PVSCLC cumulative trial ledger is inconsistent")
    return {
...
PVSCLC_CELL_SOURCE_FIELD_NAMES = frozenset(
    {
        "config_id",
        "symbol",
        "side",
...
PVSCLC_CELL_READINESS_FIELD_NAMES = frozenset(
    {
        "proxy_budget_bps",
        "cost_mode",
        "proxy_contract_ready",
        "proxy_budget_ready",
        "activity_provenance_ready",
        "source_ready",
        "source_error",
...
PVSCLC_CELL_GATE_FIELD_NAMES = frozenset(
    {
...
PVSCLC_CELL_FIELD_NAMES = (
    PVSCLC_CELL_SOURCE_FIELD_NAMES
    | PVSCLC_CELL_READINESS_FIELD_NAMES
    | PVSCLC_CELL_GATE_FIELD_NAMES
)
...
def _signal_feature_semantics_are_valid(row: Mapping[str, Any]) -> bool:
    ...
    expected_activity = value["signal_bid_volume"] + value["signal_ask_volume"]
    expected_ratio = (
        expected_activity / value["activity_v90"]
        if value["activity_v90"] > 0.0
        else math.inf
    )
    required_body = BODY_RECORDED_COST_MULTIPLE * (
        value["proxy_budget_bps"] + FROZEN_EXTRA_ROUND_TRIP_COST_BPS
    )
    if (
        row.get("activity_metric_id") != ACTIVITY_METRIC_ID
        or value["activity_v90"] <= 0.0
        or value["signal_bid_volume"] < 0.0
        or value["signal_ask_volume"] < 0.0
        or not math.isfinite(expected_activity)
        or value["signal_activity"] != expected_activity
        or value["signal_activity"] <= value["activity_v90"]
        or value["signal_activity_ratio"] != expected_ratio
        or value["signal_activity_ratio"] <= 1.0
        ...
        or not 0.0 <= value["bid_close_location"] <= 1.0
        or not 0.0 <= value["ask_close_location"] <= 1.0
        or value["bid_close_location"] < CLOSE_LOCATION_THRESHOLD
        or value["ask_close_location"] < CLOSE_LOCATION_THRESHOLD
    ):
        return False
    if row.get("side") == "BUY":
        return bool(
            value["bid_log_body_bps"] >= required_body
            and value["ask_log_body_bps"] >= required_body
        )
...
def _completed_signal_semantics_are_valid(
    ...
    if proxy is None or cap is None or signal_spread is None:
        return False
    if missing_fill:
        if any(
...
def validate_trade_row(row: Mapping[str, Any]) -> bool:
    if set(row) != PVSCLC_TRADE_FIELD_NAMES or not _all_json_numbers_finite(dict(row)):
        return False
    event_id = row.get("event_id")
    symbol = row.get("symbol")
    side = row.get("side")
...
def validate_reservation_row(row: Mapping[str, Any]) -> bool:
    ...
    expected_fields = (
        PVSCLC_MISSING_FILL_RESERVATION_FIELD_NAMES
        if missing
        else PVSCLC_RESERVATION_FIELD_NAMES
    )
    ...
    trade_row = {
        ...
        for name in PVSCLC_TRADE_FIELD_NAMES
    }
...
def validate_reservation_ledger_payload(payload: Any) -> bool:
    ...
    if isinstance(payload, Mapping):
        ...
        if (
            ...
            != "fxstack.scalp.provider_volume_shock_close_location_continuation_reservation_ledger.v1"
            or payload.get("family")
            != "provider_volume_shock_close_location_continuation"
        ):
            return False
...
def validate_trade_ledger_payload(payload: Any) -> bool:
    ...
    if isinstance(payload, Mapping):
        ...
        if (
            payload.get("schema_version")
            != "fxstack.scalp.provider_volume_shock_close_location_continuation_trade_ledger.v1"
            or payload.get("family")
            != "provider_volume_shock_close_location_continuation"
        ):
            return False
...
def validate_cells_payload(cells: Any) -> bool:
    ...
    for cell in cells:
        if not isinstance(cell, Mapping) or set(cell) != PVSCLC_CELL_FIELD_NAMES:
            return False
        ...
        keys.append(key)
    expected = {
        (CONFIG_ID, symbol, side) for symbol in FX_SYMBOLS for side in ("BUY", "SELL")
    }
...
def _cell_contract_is_valid(
    ...
    *,
    ...
    if set(cell) != PVSCLC_CELL_SOURCE_FIELD_NAMES | PVSCLC_CELL_READINESS_FIELD_NAMES:
        return False
    ...
    return bool(
        ...
        and cell.get("activity_metric_id") == ACTIVITY_METRIC_ID
        and cell.get("activity_quantile") == ACTIVITY_QUANTILE
        and cell.get("spread_quantile") == SPREAD_QUANTILE
        and cell.get("close_location_threshold") == CLOSE_LOCATION_THRESHOLD
        and cell.get("body_recorded_cost_multiple") == BODY_RECORDED_COST_MULTIPLE
...
def _apply_discovery_gate(
    *,
    ...
    for cell in cells:
        ...
        config = PVSCLC_CONFIG_BY_ID.get(key[0])
            ...
            replay.update(
                {
                    ...
                    "activity_provenance_ready": cell.get("activity_provenance_ready"),
        ...
        updates["passes_discovery_cell_gate"] = bool(
            ...
            and cell.get("activity_provenance_ready") is True
            and gate_ledger
    ...
    return {
        ...
        "all_pair_directions_same_configuration": passing == [CONFIG_ID],
...
def _has_scorable_run(bars: Sequence[QuoteBar]) -> bool:
    minimum = BASELINE_M1_BARS + FILL_DELAY_M1_BARS + OUTCOME_HORIZON_M1_BARS
    return any(
...
def _canonical_input_manifest_bytes(
    source_sha256_by_symbol: Mapping[str, str],
) -> bytes | None:
    if set(source_sha256_by_symbol) != set(FX_SYMBOLS) or any(
        not _valid_sha256(source_sha256_by_symbol.get(symbol)) for symbol in FX_SYMBOLS
    ):
        return None
    lines = [
        f"{source_sha256_by_symbol[symbol]}  input/{symbol}_M1.csv\n"
        for symbol in sorted(FX_SYMBOLS)
    ]
    return "".join(lines).encode("ascii")


def _valid_activity_provenance(
    provenance: Mapping[str, Any] | None,
    *,
    source_sha256_by_symbol: Mapping[str, str],
    source_activity_integrity_by_symbol: Mapping[str, Mapping[str, Any]],
    evaluation_start_epoch: int,
    evaluation_end_epoch: int,
    source_design_lock_epoch: int,
    preregistration_lock_epoch: int,
) -> bool:
    """Bind two-sided provider activity to exact immutable M1 inputs."""

    if provenance is None or set(provenance) != {
        "schema_version",
        "activity_metric_id",
        "provider",
        ...
        return False
    if (
        provenance.get("schema_version") != ACTIVITY_PROVENANCE_SCHEMA_VERSION
        or provenance.get("activity_metric_id") != ACTIVITY_METRIC_ID
        or provenance.get("provider") != "Dukascopy public historical feed"
        or not _valid_sha256(provenance.get("input_manifest_sha256"))
        or not _is_canonical_utc_second(provenance.get("as_of_utc"))
        or not _is_canonical_utc_second(provenance.get("acquisition_started_at_utc"))
    ):
        return False
    manifest_bytes = _canonical_input_manifest_bytes(source_sha256_by_symbol)
    if manifest_bytes is None or hashlib.sha256(
        manifest_bytes
    ).hexdigest() != provenance.get("input_manifest_sha256"):
        return False
    try:
        acquisition_started_epoch = _parse_epoch(
            str(provenance["acquisition_started_at_utc"])
        )
        as_of_epoch = _parse_epoch(str(provenance["as_of_utc"]))
    except (KeyError, TypeError, ValueError):
        return False
    if (
        acquisition_started_epoch is None
        or as_of_epoch is None
        or source_design_lock_epoch > preregistration_lock_epoch
        or acquisition_started_epoch < source_design_lock_epoch
        or as_of_epoch < acquisition_started_epoch
        or as_of_epoch > preregistration_lock_epoch
    ):
        return False

    tool = provenance.get("tool_contract")
    if not isinstance(tool, Mapping) or set(tool) != ACTIVITY_PROVENANCE_TOOL_FIELDS:
        return False
    if (
        not _valid_sha256(tool.get("tool_sha256"))
        or not _valid_sha256(tool.get("dukascopy_python_module_sha256"))
        or not _valid_sha256(tool.get("dukascopy_python_instruments_module_sha256"))
        or any(
            not isinstance(tool.get(name), str) or not str(tool[name]).strip()
            for name in (
                "tool_path",
                "dukascopy_python_version",
                "pandas_version",
        ...
        or tool.get("tool_path") != "tools/fetch_pvsclc_dual_side_volume_snapshot.py"
    ):
        return False
    ...
    if not isinstance(window, Mapping) or set(window) != {
        ...
        return False
    try:
        window_start = _parse_epoch(str(window["start_inclusive"]))
        window_end = _parse_epoch(str(window["end_exclusive"]))
    except (KeyError, TypeError, ValueError):
        return False
    if window_start != evaluation_start_epoch or window_end != evaluation_end_epoch:
        return False

    request = provenance.get("request_contract")
    if not isinstance(request, Mapping) or set(request) != {
        ...
        return False
    if (
        request.get("endpoint") != DUKASCOPY_ENDPOINT
        or request.get("timeframe") != "1MIN"
        or request.get("bid_offer_side") != "B"
        or request.get("ask_offer_side") != "A"
        or request.get("separate_requests") is not True
        or request.get("resume") is not False
        or request.get("max_retries") != 7
        or type(request.get("max_retries")) is not int
        or request.get("limit") != 5000
        or type(request.get("limit")) is not int
        or request.get("provider_end_inclusive_adjustment_minutes") != -1
        or type(request.get("provider_end_inclusive_adjustment_minutes")) is not int
        or request.get("out_of_window_rows_policy") != "reject"
    ):
        return False

    merge = provenance.get("merge_contract")
    fill = provenance.get("fill_contract")
    coverage = provenance.get("coverage_contract")
    if (
        not isinstance(merge, Mapping)
        or set(merge) != {"method", "unmatched_bid_rows", "unmatched_ask_rows"}
        or merge.get("method") != "exact_timestamp_inner_join_with_equal_side_sets"
        or merge.get("unmatched_bid_rows") != 0
        or type(merge.get("unmatched_bid_rows")) is not int
        or merge.get("unmatched_ask_rows") != 0
        or type(merge.get("unmatched_ask_rows")) is not int
        or not isinstance(fill, Mapping)
        or set(fill) != {"mid_only_fallback", "zero_fill", "synthetic_side"}
        or fill.get("mid_only_fallback") is not False
        or fill.get("zero_fill") is not False
        or fill.get("synthetic_side") is not False
        ...
        or coverage.get("rows_preserved_without_filtering") is not True
        or coverage.get("minimum_rows_per_symbol") != MINIMUM_SOURCE_ROWS_PER_SYMBOL
        or type(coverage.get("minimum_rows_per_symbol")) is not int
        or coverage.get("maximum_boundary_lag_seconds") != MAXIMUM_BOUNDARY_LAG_SECONDS
        or type(coverage.get("maximum_boundary_lag_seconds")) is not int
        or coverage.get("maximum_gap_seconds") != MAXIMUM_SOURCE_GAP_SECONDS
        or type(coverage.get("maximum_gap_seconds")) is not int
        or coverage.get("timestamp_set_hash_grammar") != TIMESTAMP_SET_HASH_GRAMMAR
    ):
        return False
    ...
    if (
        ...
        or set(symbols) != set(FX_SYMBOLS)
        or set(source_sha256_by_symbol) != set(FX_SYMBOLS)
        or set(source_activity_integrity_by_symbol) != set(FX_SYMBOLS)
    ):
        return False
    for symbol in FX_SYMBOLS:
        expected_hash = source_sha256_by_symbol.get(symbol)
        actual_integrity = source_activity_integrity_by_symbol.get(symbol)
        record = symbols.get(symbol)
        if (
            not _valid_sha256(expected_hash)
            or not isinstance(record, Mapping)
            or set(record) != ACTIVITY_PROVENANCE_SYMBOL_FIELDS
            or record.get("output_sha256") != expected_hash
            or record.get("instrument_id") != f"{symbol[:3]}/{symbol[3:]}"
            or record.get("output_file") != f"input/{symbol}_M1.csv"
            or not _valid_sha256(record.get("bid_stream_sha256"))
            or not _valid_sha256(record.get("ask_stream_sha256"))
            or not isinstance(actual_integrity, Mapping)
            or set(actual_integrity) != SOURCE_ACTIVITY_INTEGRITY_FIELDS
            or record.get("bid_stream_sha256")
            != actual_integrity.get("bid_stream_sha256")
            or record.get("ask_stream_sha256")
            != actual_integrity.get("ask_stream_sha256")
            or record.get("first_timestamp") != actual_integrity.get("first_timestamp")
            or record.get("last_timestamp") != actual_integrity.get("last_timestamp")
            or record.get("timestamp_set_sha256")
            != actual_integrity.get("timestamp_set_sha256")
            or record.get("gap_count") != actual_integrity.get("gap_count")
            or record.get("maximum_gap_seconds")
            != actual_integrity.get("maximum_gap_seconds")
        ):
            return False
        matched_rows = _exact_nonnegative_int(record.get("rows"))
        unmatched_bid = _exact_nonnegative_int(record.get("unmatched_bid_rows"))
        unmatched_ask = _exact_nonnegative_int(record.get("unmatched_ask_rows"))
        if (
            matched_rows is None
            or matched_rows < MINIMUM_SOURCE_ROWS_PER_SYMBOL
            or matched_rows != _exact_nonnegative_int(actual_integrity.get("rows"))
            or unmatched_bid != 0
            or unmatched_ask != 0
        ):
            return False
        ...
        if (
            not _is_canonical_utc_second(first_timestamp)
            or not _is_canonical_utc_second(last_timestamp)
            or not _valid_sha256(record.get("timestamp_set_sha256"))
        ):
            return False
        first_epoch = _parse_epoch(str(first_timestamp))
        last_epoch = _parse_epoch(str(last_timestamp))
        gap_count = _exact_nonnegative_int(record.get("gap_count"))
        maximum_gap = _exact_nonnegative_int(record.get("maximum_gap_seconds"))
        if (
            first_epoch is None
            or last_epoch is None
            or not evaluation_start_epoch
            <= first_epoch
            < evaluation_start_epoch + MAXIMUM_BOUNDARY_LAG_SECONDS
            or not evaluation_end_epoch - MAXIMUM_BOUNDARY_LAG_SECONDS
            <= last_epoch
            < evaluation_end_epoch
            or first_epoch > last_epoch
            or gap_count is None
            or gap_count > max(0, matched_rows - 1)
            or maximum_gap is None
            or maximum_gap > MAXIMUM_SOURCE_GAP_SECONDS
            or maximum_gap % 60 != 0
            or (matched_rows == 1 and maximum_gap != 0)
            or (matched_rows > 1 and maximum_gap < 60)
        ):
            return False
        for side_name in ("bid_volume", "ask_volume"):
            counts = record.get(side_name)
            if (
                not isinstance(counts, Mapping)
                or set(counts) != ACTIVITY_PROVENANCE_VOLUME_COUNT_FIELDS
            ):
                return False
            row_count = _exact_nonnegative_int(counts.get("rows"))
            missing_count = _exact_nonnegative_int(counts.get("missing"))
            zero_count = _exact_nonnegative_int(counts.get("zero_count"))
            expected_zero_count = _exact_nonnegative_int(
                actual_integrity.get(
                    "bid_zero_count" if side_name == "bid_volume" else "ask_zero_count"
                )
            )
            if (
                row_count != matched_rows
                or counts.get("finite") is not True
                or counts.get("nonnegative") is not True
                or missing_count != 0
                or zero_count is None
                or zero_count > matched_rows
                or zero_count != expected_zero_count
            ):
                return False
    return True
...
def screen_universe(
    *,
    ...
    activity_provenance: Mapping[str, Any] | None,
    source_sha256_by_symbol: Mapping[str, str],
    source_activity_integrity_by_symbol: Mapping[str, Mapping[str, Any]],
    evaluation_start_utc: str,
    evaluation_end_utc: str,
    source_design_lock_utc: str,
    preregistration_lock_utc: str,
    source_errors: Mapping[str, str] | None = None,
) -> dict[str, Any]:
    """Screen the exact 36-cell PVSCLC family with permanent research-only authority."""
    ...
    if preregistration_lock_epoch is None or not _is_canonical_utc_second(
        preregistration_lock_utc
    ):
        raise ValueError("preregistration_lock_utc is required")
    source_design_lock_epoch = _parse_epoch(source_design_lock_utc)
    if (
        source_design_lock_epoch is None
        or not _is_canonical_utc_second(source_design_lock_utc)
        or source_design_lock_epoch > preregistration_lock_epoch
    ):
        raise ValueError("source design lock must not follow preregistration lock")
    ...
    activity_provenance_ready = _valid_activity_provenance(
        activity_provenance,
        source_sha256_by_symbol=source_sha256_by_symbol,
        source_activity_integrity_by_symbol=source_activity_integrity_by_symbol,
        evaluation_start_epoch=evaluation_start_epoch,
        evaluation_end_epoch=evaluation_end_epoch,
        source_design_lock_epoch=source_design_lock_epoch,
        preregistration_lock_epoch=preregistration_lock_epoch,
    )
    ...
    for symbol in FX_SYMBOLS:
        ...
        if not activity_provenance_ready:
            errors[symbol] = "activity_provenance_not_ready"
            prepared = prepare_series([])
            ...
            errors[symbol] = "no_complete_271_bar_m1_run"
                ...
                cell.update(
                    {
                        ...
                        "activity_provenance_ready": activity_provenance_ready,
    ...
    return {
        "schema_version": "fxstack.scalp.provider_volume_shock_close_location_continuation_screen.v1",
        ...
        "fixed_contract": {
            "formula": (
                "provider-volume shock with executable bid/ask close-location "
                "continuation"
            ),
            "baseline": (
                "Hyndman-Fan type-7 V90 of bid_volume+ask_volume and type-7 "
                "S25 of close spread over exactly t-240 through t-1"
            ),
            "activity_metric_id": ACTIVITY_METRIC_ID,
            "activity_interpretation": (
                "Dukascopy two-sided M1 provider volume is an activity proxy, "
                "not signed order flow"
            ),
            "activity_shock": "V90>0 and activity(t)>V90 (strict)",
            "buy": (
                "bid and ask log(close/open)*1e4 >= C and both "
                "(close-low)/(high-low)>=0.80"
            ),
            "sell": (
                "bid and ask -log(close/open)*1e4 >= C and both "
                "(high-close)/(high-low)>=0.80"
            ),
            "zero_quote_range": "reject_signal_not_source_readiness_failure",
            "spread": (
                "signal close spread<=cap and exact t+1 open spread<=cap, "
                "where cap=min(S25,P)"
            ),
            "fill": "exact t+1 ask open BUY / bid open SELL",
            "recorded_cost": "S=P; C=P+1bp",
            "body_threshold": "each executable-side quote body>=1C",
            "execution_debit": "D=max(0,P-entry spread)+1bp",
            "gross_target": "T=4C",
            ...
            "cutoff_tail": (
                "expected t+1 entry must be before evaluation end; an entry "
                "inside the window with an incomplete 30-bar horizon remains "
                "an adverse unresolved reservation"
            ),
        },
        "cost_readiness": {
            "proxy_contract_ready": provenance_ready,
            ...
            "activity_provenance_ready": activity_provenance_ready,
            "activity_provenance_schema_version": (
                activity_provenance.get("schema_version")
                if isinstance(activity_provenance, Mapping)
                else None
            ),
            "activity_metric_id": ACTIVITY_METRIC_ID,
            "activity_provenance": dict(activity_provenance or {}),
            "source_sha256_by_symbol": dict(source_sha256_by_symbol),
            "source_activity_integrity_by_symbol": {
                symbol: dict(identity)
                for symbol, identity in source_activity_integrity_by_symbol.items()
            },
            ...
            "source_design_lock_utc": source_design_lock_utc,
            "source_design_lock_no_later_than_activity_as_of": (
                activity_provenance_ready
            ),
...
PVSCLC_RESULT_FIELD_NAMES = frozenset(
    {
        "schema_version",
        "family",
        "acronym",
...
def _cell_gate_fields_are_valid(
    ...
    *,
    ...
    pre_gate = {
        name: cell.get(name)
        for name in PVSCLC_CELL_SOURCE_FIELD_NAMES | PVSCLC_CELL_READINESS_FIELD_NAMES
    }
...
def validate_result_bundle(result: Mapping[str, Any]) -> bool:
    ...
    if set(result) != PVSCLC_RESULT_FIELD_NAMES or not _all_json_numbers_finite(
        dict(result)
    ):
        return False
    if (
        result.get("schema_version")
        != "fxstack.scalp.provider_volume_shock_close_location_continuation_screen.v1"
        or result.get("family") != "provider_volume_shock_close_location_continuation"
        or result.get("acronym") != "PVSCLC"
    ...
    try:
        ...
        preregistration_lock_epoch = _parse_epoch(
            str(readiness["preregistration_lock_utc"])
        )
        source_design_lock_epoch = _parse_epoch(
            str(readiness["source_design_lock_utc"])
        )
    except (KeyError, TypeError, ValueError):
        return False
    if (
        evaluation_start_epoch is None
        or evaluation_end_epoch is None
        or preregistration_lock_epoch is None
        or source_design_lock_epoch is None
        or source_design_lock_epoch > preregistration_lock_epoch
        or readiness.get("source_design_lock_no_later_than_activity_as_of") is not True
        or readiness.get("activity_provenance_ready") is not True
        or readiness.get("activity_provenance_schema_version")
        != ACTIVITY_PROVENANCE_SCHEMA_VERSION
        or readiness.get("activity_metric_id") != ACTIVITY_METRIC_ID
        or not isinstance(readiness.get("activity_provenance"), Mapping)
        or not isinstance(readiness.get("source_sha256_by_symbol"), Mapping)
        or not isinstance(readiness.get("source_activity_integrity_by_symbol"), Mapping)
        or not _valid_activity_provenance(
            readiness["activity_provenance"],
            source_sha256_by_symbol=readiness["source_sha256_by_symbol"],
            source_activity_integrity_by_symbol=readiness[
                "source_activity_integrity_by_symbol"
            ],
            evaluation_start_epoch=evaluation_start_epoch,
            evaluation_end_epoch=evaluation_end_epoch,
...
def _load_proxy_contract_bytes(
    raw: bytes,
    *,
    source_name: str,
    evaluation_start_utc: str,
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"{source_name}: expected an object")
    if set(payload) != {"schema_version", "provenance", "budgets_bps"}:
        raise ValueError(f"{source_name}: unexpected proxy contract schema")
    schema_version = payload.get("schema_version")
    if schema_version != PROXY_CONTRACT_SCHEMA_VERSION:
        raise ValueError(f"{source_name}: unexpected proxy contract version")
    ...
    if not isinstance(provenance, dict) or not _valid_proxy_provenance(
        ...
        raise ValueError(f"{source_name}: missing frozen proxy provenance")
    if not isinstance(budgets_payload, dict):
        raise ValueError(f"{source_name}: missing budgets_bps object")
...
def _load_proxy_contract(
    ...
    *,
    ...
    return _load_proxy_contract_bytes(
        path.read_bytes(),
...
def _load_activity_provenance_bytes(
    raw: bytes,
    *,
    source_name: str,
    source_sha256_by_symbol: Mapping[str, str],
    ...
    try:
        ...
        raise ValueError(
            "canonical evaluation and lock timestamps are required"
        ) from exc
    if (
        not isinstance(payload, dict)
        or evaluation_start_epoch is None
        or evaluation_end_epoch is None
        or source_design_lock_epoch is None
        or preregistration_lock_epoch is None
        or not _valid_activity_provenance(
        ...
        raise ValueError(f"{source_name}: invalid two-sided activity provenance")
    return dict(payload)


def _load_activity_provenance(
    path: Path,
    *,
    source_sha256_by_symbol: Mapping[str, str],
    source_activity_integrity_by_symbol: Mapping[str, Mapping[str, Any]],
    ...
    return _load_activity_provenance_bytes(
        path.read_bytes(),
        source_name=path.name,
        source_sha256_by_symbol=source_sha256_by_symbol,
        source_activity_integrity_by_symbol=source_activity_integrity_by_symbol,
...
def _source_activity_integrity_bytes(raw: bytes, *, source_name: str) -> dict[str, Any]:
    """Hash exact BID/ASK CSV projections without trusting provenance claims."""
    ...
    if (
        ...
        raise ValueError(f"{source_name}: noncanonical CSV byte grammar")
    try:
        text = raw.decode("ascii")
    except UnicodeDecodeError as exc:
        raise ValueError(f"{source_name}: CSV must be ASCII-compatible UTF-8") from exc
    lines = text.splitlines()
    if not lines or tuple(lines[0].split(",")) != CSV_HEADER:
        raise ValueError(f"{source_name}: unexpected M1 header")
    bid_bytes = bytearray(b"timestamp,open,high,low,close,volume\n")
    ask_bytes = bytearray(b"timestamp,open,high,low,close,volume\n")
    timestamp_bytes = bytearray(b"timestamp\n")
    bid_zero_count = 0
    ask_zero_count = 0
    ...
    for row_number, line in enumerate(lines[1:], start=2):
        fields = line.split(",")
        if len(fields) != len(CSV_HEADER) or any(
            not field or field != field.strip() for field in fields
        ):
            raise ValueError(f"{source_name}:{row_number}: noncanonical M1 row")
        timestamp = fields[0]
        if not _is_canonical_utc_second(timestamp):
            raise ValueError(f"{source_name}:{row_number}: noncanonical timestamp")
        ...
        if previous_epoch is not None:
            ...
            if gap_seconds <= 0 or gap_seconds % 60 != 0:
                raise ValueError(
                    f"{source_name}:{row_number}: invalid timestamp sequence"
                )
            gap_count += int(gap_seconds != 60)
            maximum_gap_seconds = max(maximum_gap_seconds, gap_seconds)
        previous_epoch = epoch
        if first_timestamp is None:
            first_timestamp = timestamp
        last_timestamp = timestamp
        timestamp_bytes.extend(timestamp.encode("ascii"))
        timestamp_bytes.extend(b"\n")
        try:
            bid_volume = float(fields[9])
            ask_volume = float(fields[10])
        except ValueError as exc:
            raise ValueError(f"{source_name}:{row_number}: invalid volume") from exc
        if (
            not math.isfinite(bid_volume)
            or bid_volume < 0.0
            or not math.isfinite(ask_volume)
            or ask_volume < 0.0
            or not math.isfinite(bid_volume + ask_volume)
        ):
            raise ValueError(f"{source_name}:{row_number}: invalid volume")
        bid_zero_count += int(bid_volume == 0.0)
        ask_zero_count += int(ask_volume == 0.0)
        bid_bytes.extend(
            ",".join(fields[index] for index in (0, 1, 2, 3, 4, 9)).encode("ascii")
        )
        bid_bytes.extend(b"\n")
        ask_bytes.extend(
            ",".join(fields[index] for index in (0, 5, 6, 7, 8, 10)).encode("ascii")
        )
        ask_bytes.extend(b"\n")
    rows = len(lines) - 1
    if rows <= 0:
        raise ValueError(f"{source_name}: no M1 rows")
    assert first_timestamp is not None and last_timestamp is not None
    return {
        "rows": rows,
        "bid_stream_sha256": hashlib.sha256(bid_bytes).hexdigest(),
        "ask_stream_sha256": hashlib.sha256(ask_bytes).hexdigest(),
        "bid_zero_count": bid_zero_count,
        "ask_zero_count": ask_zero_count,
        "first_timestamp": first_timestamp,
        "last_timestamp": last_timestamp,
        "timestamp_set_sha256": hashlib.sha256(timestamp_bytes).hexdigest(),
        "gap_count": gap_count,
        "maximum_gap_seconds": maximum_gap_seconds,
    }


def _source_activity_integrity(path: Path) -> dict[str, Any]:
    return _source_activity_integrity_bytes(path.read_bytes(), source_name=path.name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--source-design-lock-utc", required=True)
    parser.add_argument("--preregistration-lock-utc", required=True)
    parser.add_argument("--proxy-budgets-json", required=True)
    parser.add_argument("--activity-provenance-json", required=True)
    ...
    activity_provenance_path = Path(args.activity_provenance_json)
    proxy_raw = proxy_path.read_bytes()
    proxy_sha256 = hashlib.sha256(proxy_raw).hexdigest()
    proxy_budgets, proxy_provenance, proxy_schema_version = _load_proxy_contract_bytes(
        proxy_raw,
        source_name=proxy_path.name,
    ...
    source_activity_integrity: dict[str, dict[str, Any]] = {}
    for symbol in FX_SYMBOLS:
        ...
        try:
            raw = path.read_bytes()
            source_sha256[symbol] = hashlib.sha256(raw).hexdigest()
            source_activity_integrity[symbol] = _source_activity_integrity_bytes(
                raw, source_name=path.name
            )
            bars_by_symbol[symbol] = _load_m1_csv_bytes(
                raw,
                source_name=path.name,
                start=args.start,
                end=args.end,
    ...
    expected_manifest_bytes = _canonical_input_manifest_bytes(source_sha256)
    manifest_path = csv_root.resolve().parent / "input_sha256.txt"
    try:
        manifest_raw = manifest_path.read_bytes()
    except OSError as exc:
        raise ValueError(
            "input_sha256.txt must exactly bind the 18 input/*.csv files"
        ) from exc
    if (
        csv_root.name != "input"
        or expected_manifest_bytes is None
        or manifest_raw != expected_manifest_bytes
    ):
        raise ValueError("input_sha256.txt must exactly bind the 18 input/*.csv files")
    manifest_sha256 = hashlib.sha256(manifest_raw).hexdigest()
    activity_provenance_raw = activity_provenance_path.read_bytes()
    activity_provenance_sha256 = hashlib.sha256(activity_provenance_raw).hexdigest()
    activity_provenance = _load_activity_provenance_bytes(
        activity_provenance_raw,
        source_name=activity_provenance_path.name,
        source_sha256_by_symbol=source_sha256,
        source_activity_integrity_by_symbol=source_activity_integrity,
        evaluation_start_utc=args.start,
        evaluation_end_utc=args.end,
        source_design_lock_utc=args.source_design_lock_utc,
        preregistration_lock_utc=args.preregistration_lock_utc,
    )
    ...
    if not validate_source_replay(
        ...
        raise RuntimeError("PVSCLC complete source replay validation failed")
    result["input_metadata"] = {
        ...
        "preregistration_lock_utc": args.preregistration_lock_utc,
        "source_design_lock_utc": args.source_design_lock_utc,
        "proxy_contract_sha256": proxy_sha256,
        "activity_provenance_sha256": activity_provenance_sha256,
    ...
    if not validate_reservation_ledger_payload(reservation_payload):
        raise RuntimeError("PVSCLC reservation-ledger schema validation failed")
    if not validate_trade_ledger_payload(trade_payload):
        raise RuntimeError("PVSCLC trade-ledger schema validation failed")
    for path, payload in (
    ...
    print(
        f"PVSCLC: {len(result['cells'])} cells; "
    ...
    return (
        ...
        and readiness["activity_provenance_ready"] is True
        else 2
    )
