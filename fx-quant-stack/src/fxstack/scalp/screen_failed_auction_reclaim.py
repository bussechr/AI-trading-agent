# AGENT: ROLE: Research-only Failed Auction Reclaim bid/ask M1 discovery screen.
# AGENT: ENTRYPOINT: `screen_universe`; CLI `python -m fxstack.scalp.screen_failed_auction_reclaim`.
...
"""Causal M1 screen for the symmetric Failed Auction Reclaim (FAR).

For a closed signal bar ``t``, the volatility and spread baselines contain
exactly 240 consecutive observations ending at ``t-2``.  The volatility
baseline is the median midpoint true range and the spread baseline is the
nearest-rank Q25 (sorted index 59) of close spreads.  A BUY requires a large
negative return at ``t-1``, a positive reclaim return at ``t``, a lower-low
sweep, and a close back through the midpoint of the prior range.  SELL is the
exact mirror.
...
PRIOR_IMPULSE_VOL_THRESHOLD = 0.75
RECLAIM_RETURN_VOL_THRESHOLD = 0.25
MAX_SIGNAL_TRUE_RANGE_VOL = 2.5
FROZEN_EXTRA_ROUND_TRIP_COST_BPS = 1.0
TARGET_RECORDED_COST_MULTIPLE = 3.0
STOP_TARGET_MULTIPLE = 4.0
MAX_PROXY_SPREAD_BPS = 3.0
MAX_GROSS_STOP_BPS = 48.0
MAX_P_STAR = 13.0 / 15.0
OUTCOME_HORIZON_M1_BARS = 30
FILL_DELAY_M1_BARS = 1

IMMUTABLE_PRIOR_ATTEMPTED_CELLS = 4_314
IMMUTABLE_CURRENT_ATTEMPTED_CELLS = 36
IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS = 4_350
TWO_SIDED_BONFERRONI_FAMILY_ALPHA = 0.05
BONFERRONI_STUDENT_T_MIN_DF = 99
TWO_SIDED_BONFERRONI_STUDENT_T_MIN_DF99_ABS_THRESHOLD = 4.621681100966098
...
class FARConfig:
    """The single frozen FAR configuration."""

    prior_impulse_vol_threshold: float = PRIOR_IMPULSE_VOL_THRESHOLD
    reclaim_return_vol_threshold: float = RECLAIM_RETURN_VOL_THRESHOLD
    ...
    stop_target_multiple: float = STOP_TARGET_MULTIPLE
...
GRID: tuple[FARConfig, ...] = (FARConfig(),)
FAR_CONFIG_BY_ID: dict[str, FARConfig] = {config.config_id: config for config in GRID}
...
class FARClosedSignal:
    ...
    reclaim_return_bps: float
    reclaim_return_vol_units: float
    current_true_range_bps: float
    current_true_range_vol_units: float
    prior_mid_high: float
    prior_mid_low: float
    prior_range_midpoint: float
    signal_mid_high: float
    signal_mid_low: float
    signal_mid_close: float
...
FAR_SIGNAL_FIELD_NAMES = frozenset(FARSignal.__dataclass_fields__)
FAR_TRADE_FIELD_NAMES = frozenset(FARTrade.__dataclass_fields__)
FAR_RESERVATION_OUTCOME_FIELD_NAMES = frozenset(
    {
        "event_id",
        "reservation_status",
        "outcome_reason",
...
FAR_RESERVATION_FIELD_NAMES = (
    FAR_SIGNAL_FIELD_NAMES | FAR_RESERVATION_OUTCOME_FIELD_NAMES
)
FAR_MISSING_FILL_RESERVATION_FIELD_NAMES = FAR_RESERVATION_FIELD_NAMES | {
    "gate_risk_basis_bps"
}
...
def _build_pre_signal_context(
    ...
    for run_start, run_end in _consecutive_runs(bars):
        ...
        if (
            ...
            raise RuntimeError("FAR rolling baseline length changed")
...
def _normalized_p_star(
    *, recorded_cost_bps: float, execution_cost_debit_bps: float
    ...
    p_star = (12.0 + debit_cost_ratio) / 15.0
    if execution_cost_debit_bps == recorded_cost_bps and p_star != MAX_P_STAR:
        raise RuntimeError("FAR normalized p-star boundary lost exactness")
    if execution_cost_debit_bps <= recorded_cost_bps and p_star > MAX_P_STAR:
        raise RuntimeError("FAR normalized p-star exceeded its derived maximum")
    return p_star
...
def evaluate_closed_signal(
    *,
...
) -> tuple[FARClosedSignal | None, str]:
    """Evaluate the closed-t FAR trigger without reading or requiring t+1."""
    ...
    if prior_spread > spread_cap:
        return None, "prior_spread_above_q25_or_proxy"
    ...
    if (
        ...
        or current_tr_units > config.maximum_signal_true_range_vol
    ):
        return None, "signal_true_range_too_large"
    prior_return = _mid_close_return_bps(prepared.bars[signal_index - 2], previous_bar)
    reclaim_return = _mid_close_return_bps(previous_bar, signal_bar)
    prior_units = prior_return / context.volatility_bps
    reclaim_units = reclaim_return / context.volatility_bps
    prior_range_midpoint = (previous_bar.mid_h + previous_bar.mid_l) / 2.0
    if side == "BUY":
        if prior_units > -config.prior_impulse_vol_threshold:
            return None, "prior_impulse_too_small"
        if reclaim_units < config.reclaim_return_vol_threshold:
            return None, "reclaim_return_too_small"
        if signal_bar.mid_l >= previous_bar.mid_l:
            return None, "low_sweep_missing"
        if signal_bar.mid_c < prior_range_midpoint:
            return None, "prior_range_midpoint_not_reclaimed"
    else:
        if prior_units < config.prior_impulse_vol_threshold:
            return None, "prior_impulse_too_small"
        if reclaim_units > -config.reclaim_return_vol_threshold:
            return None, "reclaim_return_too_small"
        if signal_bar.mid_h <= previous_bar.mid_h:
            return None, "high_sweep_missing"
        if signal_bar.mid_c > prior_range_midpoint:
            return None, "prior_range_midpoint_not_reclaimed"
    ...
    return (
        FARClosedSignal(
            ...
            reclaim_return_bps=reclaim_return,
            reclaim_return_vol_units=reclaim_units,
            current_true_range_bps=current_tr,
            current_true_range_vol_units=current_tr_units,
            prior_mid_high=previous_bar.mid_h,
            prior_mid_low=previous_bar.mid_l,
            prior_range_midpoint=prior_range_midpoint,
            signal_mid_high=signal_bar.mid_h,
            signal_mid_low=signal_bar.mid_l,
            signal_mid_close=signal_bar.mid_c,
...
def _complete_signal_at_exact_next_open(
    *, prepared: PreparedSeries, closed: FARClosedSignal
) -> tuple[FARSignal | None, str]:
    ...
    gross_target = config_target = TARGET_RECORDED_COST_MULTIPLE * recorded_cost
    gross_stop = STOP_TARGET_MULTIPLE * config_target
    ...
    return (
        FARSignal(
            ...
            reclaim_return_bps=closed.reclaim_return_bps,
            reclaim_return_vol_units=closed.reclaim_return_vol_units,
            current_true_range_bps=closed.current_true_range_bps,
            current_true_range_vol_units=closed.current_true_range_vol_units,
            prior_mid_high=closed.prior_mid_high,
            prior_mid_low=closed.prior_mid_low,
            prior_range_midpoint=closed.prior_range_midpoint,
            signal_mid_high=closed.signal_mid_high,
            signal_mid_low=closed.signal_mid_low,
            signal_mid_close=closed.signal_mid_close,
...
def _event_id_fields(
    *, config_id: str, symbol: str, side: str, signal_epoch: int, entry_epoch: int
    ...
    raw = f"failed_auction_reclaim|{config_id}|{symbol}|{side}|{signal_epoch}|{entry_epoch}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:24]


def _event_id(signal: FARSignal) -> str:
...
def simulate_trade(bars: Sequence[QuoteBar], *, signal: FARSignal) -> FARTrade | None:
    ...
    raise RuntimeError("complete FAR outcome horizon did not resolve")


def _reservation_row(
    signal: FARSignal, *, status: str, trade: FARTrade | None
) -> dict[str, Any]:
    gate_pnl_r = (
        trade.pnl_r
...
def _missing_fill_reservation_row(
    closed: FARClosedSignal, *, reason: str
) -> dict[str, Any]:
    ...
    gross_stop = STOP_TARGET_MULTIPLE * gross_target
    p_star = _normalized_p_star(
        recorded_cost_bps=recorded_cost,
        execution_cost_debit_bps=execution_debit,
    )
...
def _adverse_gap_diagnostics(trades: Sequence[FARTrade]) -> dict[str, Any]:
    gap_returns = [
...
def screen_cell(
    *,
    ...
    """Generate the complete ordered source replay for one fixed FAR cell."""
    ...
    for index in range(len(prepared.bars)):
        ...
        if signal is None:
            ...
            if reason == "entry_spread_above_q25_or_proxy":
                continue
            raise RuntimeError(f"FAR completion readiness failure: {reason}")
        eligible_events += 1
        ...
        if trade is None:
            raise RuntimeError("complete FAR horizon unexpectedly failed to score")
        trades.append(trade)
    ...
    return {
        ...
        **gap_diagnostics,
        ...
        "prior_impulse_vol_threshold": config.prior_impulse_vol_threshold,
        "reclaim_return_vol_threshold": config.reclaim_return_vol_threshold,
        ...
        "stop_target_multiple": config.stop_target_multiple,
...
def trial_accounting() -> dict[str, int]:
    ...
    if current != IMMUTABLE_CURRENT_ATTEMPTED_CELLS:
        raise RuntimeError("FAR fixed-family accounting changed")
    if (
        IMMUTABLE_PRIOR_ATTEMPTED_CELLS + current
        != IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS
    ):
        raise RuntimeError("FAR cumulative trial ledger is inconsistent")
    return {
...
FAR_CELL_SOURCE_FIELD_NAMES = frozenset(
    {
        "config_id",
        "symbol",
        "side",
        ...
        "prior_impulse_vol_threshold",
        "reclaim_return_vol_threshold",
        "maximum_signal_true_range_vol",
        "target_recorded_cost_multiple",
        "stop_target_multiple",
...
FAR_CELL_READINESS_FIELD_NAMES = frozenset(
    {
        "proxy_budget_bps",
        "cost_mode",
...
FAR_CELL_GATE_FIELD_NAMES = frozenset(
    {
...
FAR_CELL_FIELD_NAMES = (
    FAR_CELL_SOURCE_FIELD_NAMES
    | FAR_CELL_READINESS_FIELD_NAMES
    | FAR_CELL_GATE_FIELD_NAMES
)
...
def _signal_feature_semantics_are_valid(row: Mapping[str, Any]) -> bool:
    numbers = {
        ...
        for name in (
            ...
            "reclaim_return_bps",
            "reclaim_return_vol_units",
            "current_true_range_bps",
            "current_true_range_vol_units",
            "prior_mid_high",
            "prior_mid_low",
    ...
    if (
        ...
        or value["current_true_range_bps"] < 0.0
        or not _numbers_match(
            value["spread_cap_bps"],
            min(value["spread_q25_bps"], value["proxy_budget_bps"]),
        )
        or value["prior_spread_bps"] > value["spread_cap_bps"]
        or value["signal_spread_bps"] > value["spread_cap_bps"]
        or not _numbers_match(
            value["prior_return_vol_units"],
            value["prior_return_bps"] / value["volatility_bps"],
        )
        or not _numbers_match(
            value["reclaim_return_vol_units"],
            value["reclaim_return_bps"] / value["volatility_bps"],
        )
        or not _numbers_match(
            value["current_true_range_vol_units"],
            value["current_true_range_bps"] / value["volatility_bps"],
        )
        or value["current_true_range_vol_units"] > MAX_SIGNAL_TRUE_RANGE_VOL
        or not _numbers_match(
            value["prior_range_midpoint"],
            (value["prior_mid_high"] + value["prior_mid_low"]) / 2.0,
        )
    ):
        return False
    if row.get("side") == "BUY":
        return bool(
            value["prior_return_vol_units"] <= -PRIOR_IMPULSE_VOL_THRESHOLD
            and value["reclaim_return_vol_units"] >= RECLAIM_RETURN_VOL_THRESHOLD
            and value["signal_mid_low"] < value["prior_mid_low"]
            and value["signal_mid_close"] >= value["prior_range_midpoint"]
        )
    return bool(
        row.get("side") == "SELL"
        and value["prior_return_vol_units"] >= PRIOR_IMPULSE_VOL_THRESHOLD
        and value["reclaim_return_vol_units"] <= -RECLAIM_RETURN_VOL_THRESHOLD
        and value["signal_mid_high"] > value["prior_mid_high"]
        and value["signal_mid_close"] <= value["prior_range_midpoint"]
    )
...
def _completed_signal_semantics_are_valid(
    ...
    if proxy is None or cap is None or prior_spread is None or signal_spread is None:
        return False
    if missing_fill:
        if any(
        ...
        expected_stress = max(prior_spread, signal_spread, entry_spread, proxy)
        if row.get("side") == "BUY":
            if not (
                _numbers_match(
    ...
    expected_stop = STOP_TARGET_MULTIPLE * expected_target
    expected_p_star = (expected_stop + expected_debit) / (
        expected_stop + expected_target
    )
    return bool(
        _numbers_match(value["spread_stress_bps"], expected_stress)
        and _numbers_match(value["recorded_cost_bps"], expected_cost)
        and _numbers_match(value["execution_cost_debit_bps"], expected_debit)
        and _numbers_match(value["gross_target_bps"], expected_target)
        and _numbers_match(value["gross_stop_bps"], expected_stop)
        and value["gross_stop_bps"] <= MAX_GROSS_STOP_BPS
        and _numbers_match(value["p_star"], expected_p_star)
        and value["p_star"] <= MAX_P_STAR
    )


def validate_trade_row(row: Mapping[str, Any]) -> bool:
    if set(row) != FAR_TRADE_FIELD_NAMES or not _all_json_numbers_finite(dict(row)):
        return False
    event_id = row.get("event_id")
    symbol = row.get("symbol")
    side = row.get("side")
    ...
    if side == "BUY":
        bracket_valid = bool(
            _numbers_match(stop_price, entry_price * (1.0 - stop / 1e4))
            and _numbers_match(target_price, entry_price * (1.0 + target / 1e4))
        )
    ...
    if reason == "time_stop" and bars_held != OUTCOME_HORIZON_M1_BARS:
        return False
    if reason in {"tp", "tp_gap_open"} and not _numbers_match(exit_price, target_price):
        return False
    if reason in {"sl", "sl_double_touch"} and not _numbers_match(
    ...
    return bool(
        bracket_valid
        and _numbers_match(pnl_bps, expected_pnl)
        and _numbers_match(pnl_r, expected_pnl / stop)
...
def validate_reservation_row(row: Mapping[str, Any]) -> bool:
    ...
    expected_fields = (
        FAR_MISSING_FILL_RESERVATION_FIELD_NAMES
        if missing
        else FAR_RESERVATION_FIELD_NAMES
    )
    ...
    if missing:
        return bool(
            ...
            and _numbers_match(row.get("gate_risk_basis_bps"), stop)
            and _numbers_match(gate_return, -(stop + debit) / stop)
            and all(
    ...
    trade_row = {
        ...
        for name in FAR_TRADE_FIELD_NAMES
    }
    return bool(
        validate_trade_row(trade_row) and _numbers_match(gate_return, row.get("pnl_r"))
    )
...
def validate_reservation_ledger_payload(payload: Any) -> bool:
    ...
    if isinstance(payload, Mapping):
        ...
        if (
            ...
            != "fxstack.scalp.failed_auction_reclaim_reservation_ledger.v1"
            or payload.get("family") != "failed_auction_reclaim"
        ):
            return False
...
def validate_trade_ledger_payload(payload: Any) -> bool:
    ...
    if isinstance(payload, Mapping):
        ...
        if (
            payload.get("schema_version")
            != "fxstack.scalp.failed_auction_reclaim_trade_ledger.v1"
            or payload.get("family") != "failed_auction_reclaim"
        ):
            return False
...
def validate_cells_payload(cells: Any) -> bool:
    ...
    for cell in cells:
        if not isinstance(cell, Mapping) or set(cell) != FAR_CELL_FIELD_NAMES:
            return False
        ...
        keys.append(key)
    expected = {
        ("far_v1", symbol, side) for symbol in FX_SYMBOLS for side in ("BUY", "SELL")
    }
...
def _cell_contract_is_valid(
    ...
    *,
    ...
    if set(cell) != FAR_CELL_SOURCE_FIELD_NAMES | FAR_CELL_READINESS_FIELD_NAMES:
        return False
    ...
    return bool(
        ...
        and cell.get("prior_impulse_vol_threshold") == PRIOR_IMPULSE_VOL_THRESHOLD
        and cell.get("reclaim_return_vol_threshold") == RECLAIM_RETURN_VOL_THRESHOLD
        ...
        and cell.get("stop_target_multiple") == STOP_TARGET_MULTIPLE
...
def _apply_discovery_gate(
    *,
    ...
    for cell in cells:
        ...
        config = FAR_CONFIG_BY_ID.get(key[0])
    ...
    return {
        ...
        "all_pair_directions_same_configuration": passing == ["far_v1"],
...
def screen_universe(
    *,
    ...
    """Screen the exact 36-cell FAR family with permanent research-only authority."""
    ...
    return {
        "schema_version": "fxstack.scalp.failed_auction_reclaim_screen.v1",
        ...
        "fixed_contract": {
            "formula": "symmetric failed-auction sweep and prior-range-midpoint reclaim",
            "baseline": (
                "median of exactly 240 midpoint true ranges and nearest-rank Q25 "
                "sorted index 59 of 240 close spreads, both ending t-2; t-242 "
                "supplies the first true-range predecessor"
            ),
            "buy": (
                "r(t-1)/V<=-0.75, r(t)/V>=0.25, mid_low(t)<mid_low(t-1), "
                "mid_close(t)>=midpoint(mid_high(t-1),mid_low(t-1))"
            ),
            "sell": (
                "r(t-1)/V>=0.75, r(t)/V<=-0.25, mid_high(t)>mid_high(t-1), "
                "mid_close(t)<=midpoint(mid_high(t-1),mid_low(t-1))"
            ),
            "signal_range": "midpoint TR(t)<=2.5V",
            "spread": (
                "t-1 close, t close, and exact t+1 open spreads each no wider "
                "than min(frozen 240-bar Q25, frozen pair proxy)"
            ),
            "fill": "exact t+1 ask open BUY / bid open SELL",
            "recorded_cost": "C=max(spread(t-1),spread(t),entry spread,proxy)+1bp",
            "execution_debit": "D=max(0,S-entry spread)+1bp",
            "gross_target": "T=3C",
            "gross_stop": "R=4T=12C and R<=48bp",
            "p_star": "(R+D)/(R+T)<=13/15",
            "outcome_horizon_m1_bars": OUTCOME_HORIZON_M1_BARS,
            "reserve_before_horizon_check": True,
            "missing_exact_fill_treatment": (
                "reserve expected t+1 day; S=proxy,C=D=proxy+1,T=3C,R=12C; "
                "gate pnl R=-13/12"
            ),
...
FAR_RESULT_FIELD_NAMES = frozenset(
    {
        "schema_version",
        "family",
        "acronym",
...
def _cell_gate_fields_are_valid(
    ...
    *,
    ...
    pre_gate = {
        name: cell.get(name)
        for name in FAR_CELL_SOURCE_FIELD_NAMES | FAR_CELL_READINESS_FIELD_NAMES
    }
...
def validate_result_bundle(result: Mapping[str, Any]) -> bool:
    ...
    if set(result) != FAR_RESULT_FIELD_NAMES or not _all_json_numbers_finite(
        dict(result)
    ):
        return False
    if (
        result.get("schema_version") != "fxstack.scalp.failed_auction_reclaim_screen.v1"
        or result.get("family") != "failed_auction_reclaim"
        or result.get("acronym") != "FAR"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not validate_source_replay(
        ...
        raise RuntimeError("FAR complete source replay validation failed")
    ...
    reservation_payload = {
        "schema_version": "fxstack.scalp.failed_auction_reclaim_reservation_ledger.v1",
        "family": result["family"],
        "reservations": result.pop("reservation_ledger"),
    }
    trade_payload = {
        "schema_version": "fxstack.scalp.failed_auction_reclaim_trade_ledger.v1",
        "family": result["family"],
        "trades": result.pop("trade_ledger"),
    }
    if not validate_reservation_ledger_payload(reservation_payload):
        raise RuntimeError("FAR reservation-ledger schema validation failed")
    if not validate_trade_ledger_payload(trade_payload):
        raise RuntimeError("FAR trade-ledger schema validation failed")
    for path, payload in (
