# AGENT: ROLE: Research-only dense volatility-compression-expansion-breakout M1 screen.
# AGENT: ENTRYPOINT: `screen_universe`; CLI `python -m fxstack.scalp.screen_volatility_compression_expansion`.
...
"""Causal M1 screen for the symmetric Volatility-Compression Expansion Breakout (VCEB).

VCEB is a two-bar continuation hypothesis evaluated only after signal bar
``t`` closes.  A consecutive 240-bar baseline ending at ``t-2`` freezes median
true range and the lower quartile of observed spread.  Bar ``t-1`` must be
compressed: its executable quote-side range is at most ``c`` frozen median
true ranges.  A BUY then requires the bid bar at ``t`` to open inside the
compressed bid range, close strictly above its high, expand to at least ``e``
...
``c in {0.50, 0.75}``, ``e in {1.25, 1.75}``, and
``q in {0.70, 0.85}``.
...
The proxy's honest artifact ``as_of_utc`` may postdate a historical evaluation,
but it must not postdate an explicit preregistration lock supplied to the run;
its causal source cutoff must independently be no later than evaluation start.
...
immutable prior ledger is 3,954 cells; these eight configs
over 18 pairs and two directions add 288, for 4,242 cumulative cells and a
dependence-robust two-sided Bonferroni Student-t absolute threshold of
4.615334703793947 at the fixed minimum 99 degrees of freedom.
...
IMMUTABLE_PRIOR_ATTEMPTED_CELLS = 3_954
IMMUTABLE_CURRENT_ATTEMPTED_CELLS = 288
IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS = 4_242
TWO_SIDED_BONFERRONI_FAMILY_ALPHA = 0.05
BONFERRONI_STUDENT_T_MIN_DF = 99
TWO_SIDED_BONFERRONI_STUDENT_T_MIN_DF99_ABS_THRESHOLD = 4.615334703793947
...
class VCEBConfig:
    compression_ceiling_vol: float
    expansion_floor_vol: float
    close_location_floor: float

    @property
    def config_id(self) -> str:
        compression = int(round(self.compression_ceiling_vol * 100.0))
        expansion = int(round(self.expansion_floor_vol * 100.0))
        close_location = int(round(self.close_location_floor * 100.0))
        return f"c{compression:02d}_e{expansion:03d}_q{close_location:02d}"


GRID: tuple[VCEBConfig, ...] = tuple(
    VCEBConfig(
        compression_ceiling_vol=compression,
        expansion_floor_vol=expansion,
        close_location_floor=close_location,
    )
    for compression in (0.50, 0.75)
    for expansion in (1.25, 1.75)
...
class VCEBClosedSignal:
    ...
    compression_range_bps: float
    compression_vol_units: float
    expansion_range_bps: float
    expansion_vol_units: float
    close_location: float
    structural_stop: float
    spread_q25_bps: float
    proxy_budget_bps: float
    compression_spread_bps: float
...
VCEB_CONFIG_BY_ID: dict[str, VCEBConfig] = {config.config_id: config for config in GRID}
VCEB_SIGNAL_FIELD_NAMES = frozenset(VCEBSignal.__dataclass_fields__)
VCEB_TRADE_FIELD_NAMES = frozenset(VCEBTrade.__dataclass_fields__)
VCEB_RESERVATION_OUTCOME_FIELD_NAMES = frozenset(
    {
        "event_id",
        "reservation_status",
        "outcome_reason",
...
VCEB_RESERVATION_FIELD_NAMES = (
    VCEB_SIGNAL_FIELD_NAMES | VCEB_RESERVATION_OUTCOME_FIELD_NAMES
)
VCEB_MISSING_FILL_RESERVATION_FIELD_NAMES = VCEB_RESERVATION_FIELD_NAMES | {
    "gate_risk_basis_bps"
}
VCEB_CELL_PRE_GATE_FIELD_NAMES = frozenset(
    {
        "config_id",
        "compression_ceiling_vol",
        "expansion_floor_vol",
...
VCEB_CELL_GATE_FIELD_NAMES = frozenset(
    {
...
def _build_pre_signal_context(
    ...
    """Build context for t from 240 bars ending at t-2 only."""
    ...
    for run_start, run_end in _consecutive_runs(bars):
        # One leading bar supplies the previous close for exactly 240 true
        # ranges.  The following t-1 compression bar and signal t are never
        # admitted to the baseline used for that signal.
...
def prepare_series(bars: Iterable[QuoteBar]) -> PreparedSeries:
    ...
    volatility, spread_q25 = _build_pre_signal_context(rows)
    return PreparedSeries(rows, volatility, spread_q25)
...
def evaluate_closed_signal(
    *,
...
) -> tuple[VCEBClosedSignal | None, str]:
    ...
    compression_bar = prepared.bars[signal_index - 1]

    compression_spread = compression_bar.spread_close_bps
    signal_spread = signal_bar.spread_close_bps
    spread_cap = min(context.spread_q25_bps, budget)
    if compression_spread > spread_cap + 1e-12:
        return None, "compression_spread_above_q25_or_proxy"
    if signal_spread > spread_cap + 1e-12:
        return None, "signal_spread_above_q25_or_proxy"

    if side == "BUY":
        compression_range_bps = (
            (compression_bar.bid_h - compression_bar.bid_l)
            / compression_bar.bid_c
            * 1e4
        )
        breakout_level = compression_bar.bid_h
        crossed_from_inside = (
            compression_bar.bid_l <= signal_bar.bid_o <= breakout_level
            and signal_bar.bid_c > breakout_level
        )
        quote_range = signal_bar.bid_h - signal_bar.bid_l
        expansion_range_bps = quote_range / signal_bar.bid_c * 1e4
        close_location = (
        ...
        compression_range_bps = (
            (compression_bar.ask_h - compression_bar.ask_l)
            / compression_bar.ask_c
            * 1e4
        )
        breakout_level = compression_bar.ask_l
        crossed_from_inside = (
            breakout_level <= signal_bar.ask_o <= compression_bar.ask_h
            and signal_bar.ask_c < breakout_level
        )
        quote_range = signal_bar.ask_h - signal_bar.ask_l
        expansion_range_bps = quote_range / signal_bar.ask_c * 1e4
        close_location = (
    ...
    compression_units = compression_range_bps / context.volatility_bps
    if (
        not math.isfinite(compression_units)
        or compression_units > config.compression_ceiling_vol + 1e-12
    ):
        return None, "t_minus_1_not_compressed"
    if not crossed_from_inside:
        return None, "compressed_bar_not_broken_from_inside"
    expansion_units = expansion_range_bps / context.volatility_bps
    if (
        not math.isfinite(expansion_units)
        or expansion_units + 1e-12 < config.expansion_floor_vol
    ):
        return None, "signal_range_not_expanded"
    ...
    return (
        VCEBClosedSignal(
            ...
            compression_range_bps=compression_range_bps,
            compression_vol_units=compression_units,
            expansion_range_bps=expansion_range_bps,
            expansion_vol_units=expansion_units,
            close_location=close_location,
            structural_stop=structural_stop,
            spread_q25_bps=context.spread_q25_bps,
            proxy_budget_bps=budget,
            compression_spread_bps=compression_spread,
...
def _complete_signal_at_exact_next_open(
    *,
    prepared: PreparedSeries,
    closed: VCEBClosedSignal,
) -> tuple[VCEBSignal | None, str]:
    ...
    return (
        VCEBSignal(
            ...
            breakout_level=closed.breakout_level,
            compression_range_bps=closed.compression_range_bps,
            compression_vol_units=closed.compression_vol_units,
            expansion_range_bps=closed.expansion_range_bps,
            expansion_vol_units=closed.expansion_vol_units,
            ...
            compression_spread_bps=closed.compression_spread_bps,
...
def _event_id_fields(
    *,
    ...
    raw = (
        f"volatility_compression_expansion_breakout|{config_id}|{symbol}|{side}|"
        f"{signal_epoch}|{entry_epoch}"
    )
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:24]


def _event_id(signal: VCEBSignal) -> str:
...
def simulate_trade(bars: Sequence[QuoteBar], *, signal: VCEBSignal) -> VCEBTrade | None:
...
def _reservation_row(
    signal: VCEBSignal, *, status: str, trade: VCEBTrade | None
) -> dict[str, Any]:
    gate_pnl_r = (
        trade.pnl_r
...
def _missing_fill_reservation_row(
    closed: VCEBClosedSignal, *, reason: str
) -> dict[str, Any]:
    conservative_cost_debit = closed.proxy_budget_bps + FROZEN_EXTRA_ROUND_TRIP_COST_BPS
    ...
    return {
        ...
        "pnl_bps": None,
        "pnl_r": None,
        "gate_pnl_r": -(gate_risk_basis + conservative_cost_debit) / gate_risk_basis,
        "gate_treatment": (
...
def screen_cell(
    *,
    ...
    return {
        ...
        "gate_total_r": math.fsum(gate_pnl_rs),
...
def trial_accounting() -> dict[str, int]:
    ...
    if current != IMMUTABLE_CURRENT_ATTEMPTED_CELLS:
        raise RuntimeError("VCEB fixed-grid accounting changed")
    if (
        IMMUTABLE_PRIOR_ATTEMPTED_CELLS + IMMUTABLE_CURRENT_ATTEMPTED_CELLS
        != IMMUTABLE_CUMULATIVE_ATTEMPTED_CELLS
    ):
        raise RuntimeError("VCEB cumulative trial ledger is inconsistent")
    return {
    ...
    stable = bool(
        ...
        and all(
            ...
            for segment_count, segment_wins, total in zip(
                counts, wins, totals, strict=True
            )
        )
    )
    return {
...
def _signal_feature_semantics_are_valid(
    ...
    if not _event_identity_is_valid(row, key=key):
        return False
    config = VCEB_CONFIG_BY_ID.get(key[0])
    volatility = _finite_float(row.get("volatility_bps"))
    breakout_level = _finite_float(row.get("breakout_level"))
    structural_stop = _finite_float(row.get("structural_stop"))
    compression_range = _finite_float(row.get("compression_range_bps"))
    compression_units = _finite_float(row.get("compression_vol_units"))
    expansion_range = _finite_float(row.get("expansion_range_bps"))
    expansion_units = _finite_float(row.get("expansion_vol_units"))
    ...
    compression_spread = _finite_float(row.get("compression_spread_bps"))
    ...
    if (
        ...
        or volatility <= 0.0
        or breakout_level is None
        or breakout_level <= 0.0
        or structural_stop is None
        or structural_stop <= 0.0
        or compression_range is None
        or compression_range < 0.0
        or compression_units is None
        or expansion_range is None
        or expansion_range <= 0.0
        or expansion_units is None
        or close_location is None
        or spread_q25 is None
        or spread_q25 <= 0.0
        or proxy_budget is None
        or proxy_budget <= 0.0
        or compression_spread is None
        or compression_spread < 0.0
    ...
    return bool(
        _numbers_match(compression_units, compression_range / volatility)
        and compression_units <= config.compression_ceiling_vol + 1e-12
        and _numbers_match(expansion_units, expansion_range / volatility)
        and expansion_units + 1e-12 >= config.expansion_floor_vol
        and config.close_location_floor - 1e-12 <= close_location <= 1.0 + 1e-12
        and compression_spread <= spread_cap + 1e-12
        and signal_spread <= spread_cap + 1e-12
        and (
            (key[2] == "BUY" and structural_stop <= breakout_level + 1e-12)
            or (key[2] == "SELL" and structural_stop >= breakout_level - 1e-12)
        )
...
def _completed_signal_semantics_are_valid(
    ...
    return bool(
        ...
        and _numbers_match(risk, expected_risk)
        ...
        and _numbers_match(incremental_stress, expected_incremental)
        and _numbers_match(debit, expected_debit)
        and _numbers_match(recorded_cost, expected_recorded)
...
def _trade_semantics_are_valid(
    ...
    if set(trade) != VCEB_TRADE_FIELD_NAMES or not _event_identity_is_valid(
    ...
    if (
        ...
        or pnl_bps is None
        or pnl_r is None
        or not _numbers_match(pnl_r, pnl_bps / risk_bps)
        or exit_reason
        not in {
            "sl_gap_open",
            "tp_gap_open",
        ...
        return False
    positive_outcome = pnl_bps > 0.0
    full_target_win = bool(exit_reason in {"tp", "tp_gap_open"} and positive_outcome)
    return bool(
...
def _scored_reservation_semantics_are_valid(
    ...
    *,
    ...
    if (
        trade is None
        or set(reservation) != VCEB_RESERVATION_FIELD_NAMES
    ...
    bracket_exit_matches = bool(
        (
            reason not in {"tp", "tp_gap_open"}
            or _numbers_match(exit_price, target_price)
        )
        and (
            reason not in {"sl", "sl_double_touch"}
            or _numbers_match(exit_price, stop_price)
        )
        and (
            reason != "sl_gap_open"
    ...
    return bool(
        ...
        and _numbers_match(stop_risk_bps, risk_bps)
        and _numbers_match(target_bps, risk_bps * REWARD_RISK)
        and _numbers_match(reservation.get("quote_target_bps"), target_bps)
        and _numbers_match(reservation.get("pnl_bps"), gross_bps - debit)
        and _numbers_match(
            reservation.get("gate_pnl_r"),
            trade.get("pnl_r"),
        )
    )
...
def _unresolved_reservation_semantics_are_valid(
    ...
    reason = reservation.get("outcome_reason")
    expected_schema = (
        VCEB_MISSING_FILL_RESERVATION_FIELD_NAMES
        if reason in {"exact_next_open_unavailable", "exact_next_open_gap"}
        else VCEB_RESERVATION_FIELD_NAMES
    )
    if (
    ...
    gate_pnl_r = _finite_float(reservation.get("gate_pnl_r"))
    debit = _finite_float(reservation.get("execution_cost_debit_bps"))
    if gate_pnl_r is None or gate_pnl_r >= 0.0 or debit is None or debit <= 0.0:
        return False
    ...
    if risk_basis is None or risk_basis <= 0.0 or treatment != expected_treatment:
        return False
    expected_gate_pnl_r = -(risk_basis + debit) / risk_basis
    return _numbers_match(gate_pnl_r, expected_gate_pnl_r)
    ...
    *,
    ...
    if signal is None:
        ...
        expected_reservation = _missing_fill_reservation_row(
            closed, reason=completion_reason
        )
        return bool(
            trade is None
            and _mapping_matches_exact_generated_row(reservation, expected_reservation)
        )
...
def _cell_contract_is_valid(
    ...
    *,
    key: tuple[str, str, str],
    config: VCEBConfig | None,
) -> bool:
    fields = frozenset(cell)
    if fields not in {
        VCEB_CELL_PRE_GATE_FIELD_NAMES,
        VCEB_CELL_PRE_GATE_FIELD_NAMES | VCEB_CELL_GATE_FIELD_NAMES,
    }:
        return False
...
def _apply_discovery_gate(
    *,
    ...
    for reservation in reservation_ledger:
        reservations_by_cell.setdefault(_ledger_cell_key(reservation), []).append(
            reservation
        )
    for trade in trade_ledger:
        trades_by_cell.setdefault(_ledger_cell_key(trade), []).append(trade)
    ...
    trade_id_rows = [str(trade.get("event_id") or "") for trade in trade_ledger]
    global_trade_identity_matches = bool(
        ledger_keys_are_known
        and all(scored_id_rows)
        and all(trade_id_rows)
        and len(scored_id_rows) == len(set(scored_id_rows))
        and len(trade_id_rows) == len(set(trade_id_rows))
        and Counter(scored_id_rows) == Counter(trade_id_rows)
    )
    ...
    for cell in cells:
        ...
        fixed_config = VCEB_CONFIG_BY_ID.get(key[0])
        cell_contract_consistent = _cell_contract_is_valid(
            cell, key=key, config=fixed_config
        )
        ...
        cell_trades = trades_by_cell.get(key, [])
        temporal_scope_consistent = bool(
            ...
            and all(
                ...
                for row in (*reservations, *cell_trades)
            )
        )
        scored_reservations = [
        ...
        statuses_are_exact = bool(
            len(scored_reservations) + len(unresolved_reservations) == len(reservations)
        )
        trade_by_event_id = {str(row.get("event_id") or ""): row for row in cell_trades}
        prepared_for_symbol = (
        ...
        trade_ids = [str(row.get("event_id") or "") for row in cell_trades]
        scored_ids = [str(row.get("event_id") or "") for row in scored_reservations]
        per_cell_trade_identity_matches = bool(
            all(reservation_ids)
            and all(trade_ids)
        ...
        semantic_rows_are_valid = bool(
            all(
                _scored_reservation_semantics_are_valid(
                    row,
                    trade=trade_by_event_id.get(str(row.get("event_id") or "")),
                    key=key,
                )
                for row in scored_reservations
            )
            and all(
                _unresolved_reservation_semantics_are_valid(row, key=key)
                for row in unresolved_reservations
            )
            and all(
                _reservation_matches_source_replay(
                    row,
                    trade_by_event_id.get(str(row.get("event_id") or "")),
                    key=key,
                    prepared=prepared_for_symbol,
                    proxy_budget_bps=expected_proxy_budget,
                )
        ...
        gate_returns = [value for value in parsed_gate_returns if value is not None]
        wins = sum(row.get("full_target_win") is True for row in scored_reservations)
        positive_outcomes = sum(
            row.get("positive_outcome") is True for row in scored_reservations
        )
        parsed_pnl_rs = [_finite_float(row.get("pnl_r")) for row in cell_trades]
        pnl_rs = [value for value in parsed_pnl_rs if value is not None]
        reservations_count = len(reservations)
        scored_count = len(scored_reservations)
        unresolved_count = len(unresolved_reservations)
        expected_trade_rate = wins / scored_count if scored_count else 0.0
        expected_reservation_rate = (
            wins / reservations_count if reservations_count else 0.0
        )
        expected_positive_rate = (
            positive_outcomes / scored_count if scored_count else 0.0
        )
        aggregate_fields_match = bool(
            _exact_nonnegative_int(cell.get("entry_day_reservations"))
            == reservations_count
            and _exact_nonnegative_int(cell.get("eligible_events"))
            == reservations_count
            and _exact_nonnegative_int(cell.get("scored_trades")) == scored_count
            and _exact_nonnegative_int(cell.get("unresolved_reservations"))
            == unresolved_count
            and _exact_nonnegative_int(cell.get("full_target_wins")) == wins
            and _exact_nonnegative_int(cell.get("positive_outcomes"))
            == positive_outcomes
            and cell.get("reservation_event_ids") == reservation_ids
            and cell.get("trade_event_ids") == trade_ids
            and len(pnl_rs) == len(cell_trades)
            and cell.get("exit_mix")
            == dict(Counter(str(row.get("exit_reason")) for row in cell_trades))
            and _numbers_match(
            ...
            and _numbers_match(
                cell.get("mean_r"),
                statistics.fmean(pnl_rs) if pnl_rs else 0.0,
            )
        ...
        ledger_consistent = bool(
            ...
            and _numbers_match(cell_proxy_budget, expected_proxy_budget)
            and _numbers_match(
                cell.get("compression_ceiling_vol"),
                fixed_config.compression_ceiling_vol,
            )
            and _numbers_match(
                cell.get("expansion_floor_vol"),
                fixed_config.expansion_floor_vol,
            )
            and _numbers_match(
                cell.get("close_location_floor"),
                fixed_config.close_location_floor,
            )
            and ledger_keys_are_known
            and statuses_are_exact
            and per_cell_trade_identity_matches
            and semantic_rows_are_valid
            and aggregate_fields_match
            and len(set(days)) == len(days)
            and all(days)
            and len(gate_returns) == len(reservations)
            and len(pnl_rs) == len(cell_trades)
            and all(
                _numbers_match(row.get("proxy_budget_bps"), expected_proxy_budget)
                for row in reservations
            )
            and global_trade_identity_matches
        )
        independent_days = len(set(days))
        reservation_rate = expected_reservation_rate
        wilson_lower = _one_sided_wilson_lower_bound(wins, reservations_count)
        ...
        passes = bool(
            ...
            and scored_count >= MIN_TRADES_PER_CELL
        ...
        cell.update(
            {
                ...
                **temporal_thirds,
                ...
                "gate_actual_scored_reservations": scored_count,
                "gate_actual_unresolved_reservations": unresolved_count,
                "gate_scored_trade_ids_match": per_cell_trade_identity_matches,
                "gate_row_semantics_consistent": semantic_rows_are_valid,
                ...
                "gate_proxy_contract_binding_consistent": bool(
                    ...
                    and all(
                        _numbers_match(
                            row.get("proxy_budget_bps"), expected_proxy_budget
                        )
                        for row in reservations
                    )
                ),
    ...
    return {
        ...
        "wilson_scope": (
            ...
            "two-sided Bonferroni Student-t threshold over all 4242 cumulative "
...
def _has_scorable_run(bars: Sequence[QuoteBar]) -> bool:
    minimum = BASELINE_M1_BARS + 1 + 1 + 1 + OUTCOME_HORIZON_M1_BARS
    return any(
...
def _valid_proxy_provenance(
    ...
    *,
    ...
    return bool(
        ...
        and as_of_epoch <= preregistration_lock_epoch
    )
...
def screen_universe(
    *,
    ...
    return {
        "schema_version": "fxstack.scalp.volatility_compression_expansion_breakout_screen.v1",
        "family": "volatility_compression_expansion_breakout",
        "acronym": "VCEB",
        "research_only": True,
        ...
        "fixed_contract": {
            "formula": (
                "t-1 BUY-bid / SELL-ask range <= c*median baseline TR; t opens "
                "inside that quote-side range, closes beyond it with quote-side "
                "range >= e*median baseline TR and directional close location >= q"
            ),
            "baseline": (
                "median of exactly 240 M1 true ranges ending t-2; one extra "
                "leading bar supplies the first previous close, and t-1 is held out"
            ),
            "compression": (
                "t-1 executable quote-side high-low range divided by frozen median TR"
            ),
            "expansion_breakout": (
                "t executable quote-side high-low range divided by frozen median TR; "
                "open inside and close beyond the t-1 executable quote-side range"
            ),
            "spread": (
                "Q25 of the exact same 240 baseline close spreads ending t-2; "
                "t-1 close, t close, and exact t+1 open must each be no wider "
                "than min(Q25, frozen pair proxy)"
            ),
            "proxy_time_contract": (
                "source_cutoff_utc <= evaluation_start_utc; honest artifact "
                "as_of_utc <= explicit preregistration_lock_utc. Historical "
                "evaluation does not require falsely backdating artifact creation"
            ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    reservation_payload = {
        "schema_version": (
            "fxstack.scalp.volatility_compression_expansion_breakout_reservation_ledger.v1"
        ),
    ...
    trade_payload = {
        "schema_version": (
            "fxstack.scalp.volatility_compression_expansion_breakout_trade_ledger.v1"
        ),
        "family": result["family"],
