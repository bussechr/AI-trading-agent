...
All MCP services are disabled by default and support stdio only. Disabled servers may be described but refuse resource/prompt/tool requests and do not enter the stdio loop. OpenClaw is also disabled by default; disabled construction is filesystem-inert, while enabled construction requires sandboxing and exposes only its staging workspace. It cannot invoke offline replay/research tooling or read research roots. On Windows, use `ops\windows\26_operator_plane.bat describe` for a read-only capability check.
