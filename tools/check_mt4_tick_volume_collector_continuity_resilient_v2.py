...
changing their v1 source.  It binds them to the final v3 collector executable,
its exact wrapper and base support sources, a distinct immutable guard identity,
and the embedded v3 late-gap contract.  The helper is read-only except for
...
# AGENT: HANDSHAKE: exact v3 collector/prereg/output/config identity ->
# gap-v3 Windows guard.
# AGENT: ISOLATION: no credential read, API call, evaluation, issuer, runtime,
# activation, or trade path.
# AGENT: SIDE EFFECTS: optional exclusive publication of one immutable gap-v3
# guard identity.
import argparse
import hashlib
import json
...
from tools import capture_ig_mt4_m1_activity_resilient_v3 as collector_v3
from tools import (
    check_mt4_tick_volume_collector_continuity_resilient as core,
)

TOOL_PATH = Path(__file__).resolve()
CORE_PATH = Path(core.__file__).resolve()
GUARD_SCHEMA_VERSION = "fxstack.mtvclc_collector_guard_identity.gap_v3.v1"
INSPECTION_SCHEMA_VERSION = (
    "fxstack.mtvclc_collector_continuity_inspection.gap_v3.v1"
)
GUARD_IDENTITY_FILENAME = "collector-guard.identity.gap-v3.v1.json"


class _CollectorCompatibility:
    """Expose v3 identity plus the pinned reusable ledger implementation."""

    _SPECIAL: ClassVar[dict[str, Any]] = {
        # The reusable core calls this the preserved support source.  V3 has
        # two immutable dependencies: its v2 wrapper and the original base.
        # Active-journal records deliberately bind the base source here; the
        # v3-specific identity below binds both sources separately.
        "PRESERVED_SUPPORT_PATH": collector_v3.BASE_SUPPORT_PATH,
        "PRESERVED_SUPPORT_SHA256": collector_v3.BASE_SUPPORT_SHA256,
        "PRESERVED_SUPPORT_SIZE_BYTES": collector_v3.BASE_SUPPORT_SIZE_BYTES,
    }

    def __getattr__(self, name: str) -> Any:
        if name in self._SPECIAL:
            return self._SPECIAL[name]
        if hasattr(collector_v3, name):
            return getattr(collector_v3, name)
        return getattr(collector_v3.support, name)
...
# The reused validators resolve these globals when called.  Keeping the
# adapter in this module avoids modifying or weakening the v1 inspector.
...
def _inspect_start_edge_durable_receipt(
    ...
    *,
    ...
    if bool(manifest["manifest_present"]):
        manifest_path = output_root / collector_v3.MANIFEST_FILENAME
    ...
    receipt_path = output_root / collector_v3.START_EDGE_RECEIPT_FILENAME
    ...
    try:
        proof = collector_v3.validate_start_edge_durability_receipt(
            output_root,
            binding=binding,
            ledger=view,  # type: ignore[arg-type]
        )
    except collector_v3.CollectionRefusal as exc:
...
def _capture_data_artifact_present(output_root: Path) -> bool:
    known_files = (
        collector_v3.MANIFEST_FILENAME,
        collector_v3.ACTIVE_JOURNAL_FILENAME,
        collector_v3.START_EDGE_RECEIPT_FILENAME,
        collector_v3.TAIL_COMMITMENT_FILENAME,
        collector_v3.DATA_WRITER_LOCK_FILENAME,
    )
    if any((output_root / name).exists() for name in known_files):
        return True
    chunk_root = output_root / collector_v3.support.CHUNK_DIRECTORY
...
def _inspect_capture_tail_commitment(
    ...
    *,
    ...
    tail_path = output_root / collector_v3.TAIL_COMMITMENT_FILENAME
    ...
    try:
        proof = collector_v3.validate_tail_commitment_registry(output_root)
    except collector_v3.CollectionRefusal as exc:
        raise ContinuityRefusal(str(exc)) from None
    expected_binding = {
    ...
    if (
        ...
        != collector_v3.TAIL_COMMITMENT_SCHEMA_VERSION
        or proof.get("filename") != collector_v3.TAIL_COMMITMENT_FILENAME
        or proof.get("collector_source_sha256") != collector_v3.MODULE_SOURCE_SHA256
        or proof.get("collector_wrapper_source_sha256")
        != collector_v3.SUPPORT_SHA256
        or proof.get("collector_base_source_sha256")
        != collector_v3.BASE_SUPPORT_SHA256
        or (
    ...
    identity.update(
        {
            ...
            "collector_wrapper_source_path": str(collector_v3.SUPPORT_PATH),
            "collector_wrapper_source_sha256": collector_v3.SUPPORT_SHA256,
            "collector_base_source_path": str(collector_v3.BASE_SUPPORT_PATH),
            "collector_base_source_sha256": collector_v3.BASE_SUPPORT_SHA256,
            "start_edge_durable_receipt_path": str(
                output_root / collector_v3.START_EDGE_RECEIPT_FILENAME
            ),
            "capture_tail_commitment_contract": {
                "schema_version": collector_v3.TAIL_COMMITMENT_SCHEMA_VERSION,
                "path": str(output_root / collector_v3.TAIL_COMMITMENT_FILENAME),
                "validator_api": "validate_tail_commitment_registry",
                "collector_source_sha256": collector_v3.MODULE_SOURCE_SHA256,
            ...
            "resume_contract": {
                ...
                "maximum_start_edge_lag_seconds": (
                    collector_v3.MAXIMUM_START_EDGE_LAG_SECONDS
                ),
...
def inspect_continuity(
    *,
    ...
    collector_path = collector_v3.TOOL_PATH.resolve(strict=True)
    expected_collector_path = (
        REPOSITORY_ROOT / "tools" / "capture_ig_mt4_m1_activity_resilient_v3.py"
    ).resolve()
    ...
    try:
        binding = collector_v3.load_preregistration(
        ...
        normalized_url = collector_v3.support._validated_loopback_base_url(base_url)
    except collector_v3.CollectionRefusal as exc:
        raise ContinuityRefusal(str(exc)) from None
    ...
    identity = _gap_v3_guard_identity(
        collector_path=collector_path,
        preregistration_path=preregistration_path,
        output_root=output_root,
