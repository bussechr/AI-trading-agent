"""Offline evaluator adapter for an independent MTVCLC v5 capture.

The exact v3 evaluator is retained and bound only to the v5 handoff.  It
cannot evaluate before the sealed end and exposes no network, signing,
runtime, broker, or trade authority.
"""

from __future__ import annotations

# AGENT: ROLE: isolated post-window evaluator for v5 evidence.
# AGENT: HANDSHAKE: exact v5 handoff -> unchanged v3 ledger evaluator.
# AGENT: ISOLATION: immutable local files only; no live or authority surface.
...
EVALUATOR_TOOL_REVISION = "fxstack.scalp.mtvclc_runtime_bound_evaluator_tool.v5"
DERIVATION_MODE = "exact_v3_evaluator_template_plus_counted_literal_v5_transform"
EXPECTED_V3_TEMPLATE_SHA256 = (
    "066d165d7a66f29d8783be33220e3926e9bff95b24e86fefa43c80f8dc3c4259"
)
EXPECTED_V3_TEMPLATE_SIZE_BYTES = 14_721
_MAXIMUM_SOURCE_BYTES = 8 * 1024 * 1024


class V5EvaluatorBootstrapRefusal(RuntimeError):
    """Raised before the pinned evaluator implementation is available."""
...
def _read_exact_source(path: Path, *, reason: str) -> tuple[bytes, tuple[int, ...]]:
    ...
    try:
        ...
        raise V5EvaluatorBootstrapRefusal(reason) from exc
    identities = {
    ...
    if len(identities) != 1 or len(raw) != before_handle.st_size:
        raise V5EvaluatorBootstrapRefusal(reason)
...
def _self_source() -> tuple[bytes, tuple[int, ...]]:
    ...
    return _read_exact_source(TOOL_PATH, reason="v5_evaluator_source_invalid")
...
def _assert_pinned_v3_template(raw: bytes) -> None:
    if (
        ...
        raise V5EvaluatorBootstrapRefusal("v3_evaluator_template_identity_invalid")
...
_IMPLEMENTATION_NAME = "_fxstack_mtvclc_runtime_bound_evaluator_v5_impl"
...
try:
    exec(  # noqa: S102 - pinned template bytes plus one counted transform
        compile(_DERIVED_SOURCE, str(TOOL_PATH), "exec", dont_inherit=True),
        _implementation.__dict__,
    )
except Exception as exc:
    raise V5EvaluatorBootstrapRefusal(
        "v5_evaluator_derived_source_import_invalid"
    ) from exc
finally:
