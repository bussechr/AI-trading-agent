...
- Live startup additionally reads trust policy only from the fixed OS path and checks the runtime SID/principal and pinned files. Policy booleans cannot prove isolation: preflight performs read-only PostgreSQL role/membership checks and reads the fresh database-backed EA lease established by an authenticated command poll. Without independently observed least-privilege DB grants, singleton bridge consumer, terminal-wide EA lease, distinct poll/ACK token scope, credential generation, and research-credential absence, startup reports `physical_boundary_unproven`. Staged-safe startup does not require those live-only capabilities.
