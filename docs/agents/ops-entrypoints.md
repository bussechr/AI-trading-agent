...
- End-to-end proof uses the single installed runner in shadow posture and requires repository-owned listeners plus explicit heartbeat/tick freshness evidence. Set `FXSTACK_CANDIDATE_FEATURE_ROOT` to the candidate's isolated `features` directory; the runner keeps refreshed raw bars in its sibling `raw` directory and never writes the production data tree during that proof.
...
- Direct `python -I -m fxstack.runtime.runner` invocation cannot bypass startup admission: Python repeats settings validation, profile/mode/live arming and explicit-scope checks, rejects the adaptive observation twin in live posture, requires binding structure-timing/chase, uncertainty, belief, campaign, and capital-governance settings, proves the complete forbidden-module set is absent, and runs the same read-only model preflight before bridge checks or `RuntimeService` access.
...
- Live posture is never inferred: it requires `FXSTACK_START_PROFILE=live`, an explicitly matching `FXSTACK_AGENT_MODE=live`, `FXSTACK_LIVE_ARMED=1`, an explicit `FXSTACK_LIVE_EXPECTED_ACCOUNT_MODE=demo` or `real`, and explicit non-empty `FXSTACK_AGENT_LIVE_PAIR_ALLOWLIST`, `FXSTACK_AGENT_LIVE_SLEEVE_ALLOWLIST`, and `FXSTACK_AGENT_LIVE_INTENT_ALLOWLIST` values. It also requires structure/chase, uncertainty, belief hard-gate, campaign, and capital-governance producers to be binding. Both Python settings and `_env.bat` default `FXSTACK_ADAPTIVE_SHADOW_ENABLED=0`, observation requires explicit non-live opt-in, and Python rejects live startup if that twin is enabled. Direct intelligence is controlled independently by `FXSTACK_ADAPTIVE_EXECUTION_ENABLED`; when enabled it owns adaptive ranking and sleeve governance without any shadow dependency. `_env.bat` never chooses the expected broker mode or live scopes; any missing or disabling override makes Python startup fail closed.
...
- Entry-canary checks apply only to new `enter` authority. Protective lifecycle actions do not depend on entry-canary activity, but they still require their explicit live intent scope and remain subject to runtime/queue kills, governance, risk, reconciliation, and broker validation.
...
- `21_start_runtime.bat` passes the explicit `FXSTACK_RUNTIME_FEATURE_ROOT` into the installed runner. `24_start_candidate_stack.bat` maps `FXSTACK_CANDIDATE_FEATURE_ROOT` onto that contract so candidate raw and feature writes remain physically separate from baseline data.
...
- Python startup admission -> settings/posture, live observation-twin exclusion, physical research-package exclusion, and read-only active-manifest identity before bridge/service access
