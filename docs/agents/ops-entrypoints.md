...
- Research and release workflows may export/import signed content-addressed evidence for operator review and future deployment decisions. In production this state is advisory: it cannot arm egress and its absence, rejection, expiry, or drift cannot block the live runtime.
...
- `ops/windows/provision_live_db_boundary.sql` is the administrator-reviewed, idempotent local role reduction for the default deployment: it removes cluster-wide creation authority from `fx` and creates a separate `NOLOGIN` research marker without granting membership. Run it through `psql` as a PostgreSQL administrator; the runtime account cannot self-authorize this boundary.
...
- Live validation also requires finite positive sizing and hard-risk limits before any process reset or spawn. The shipped low-risk defaults scale at `0.00001` lots/USD, cap each order at `0.10` lots, stop new entries at `5%` drawdown, and cap gross/net lot exposure at `0.30`/`0.20`; zero is never interpreted as “unbounded” in an execution posture. `validate_runtime_risk_limits.ps1` requires the managed-runner TP multiple to be either disabled at `0` or finite and at least `1R`; Python startup repeats the contract through `Settings.validate_for_startup()`.
- With lifecycle actions enabled, `_env.bat` defaults `FXSTACK_MANAGED_RUNNER_TP_R_MULTIPLE=4.0`. The runtime retains the existing SL calculation and sends the broker a TP at least `4R` away as a fail-safe; position-signature-stable lifecycle state, final partial materialization, and final lifecycle risk approval own ordinary partials and exits.
