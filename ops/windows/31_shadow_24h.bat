...
echo [shadow-24h] baseline=%BASELINE_URL% candidate=%CANDIDATE_URL%
"%TRADER_PYTHON_EXE%" -m src.trader.cli scenario shadow-run -- --baseline-url %BASELINE_URL% --candidate-url %CANDIDATE_URL% --duration-secs %DURATION_SECS% --poll-secs 2 --min-throughput-delta 1 --max-timeout-rate 0.01 --require-nonzero-entries --rollback-on-fail --rollback-cmd "%ROLLBACK_CMD%" --out-dir %OUT_DIR% --prefix canary_shadow_24h
set "RC=%ERRORLEVEL%"
echo [shadow-24h] exit_code=%RC%
exit /b %RC%
