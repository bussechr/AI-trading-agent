...
for %%P in (%FXSTACK_PAIRS_SP%) do (
  ...
  "%TRADER_PYTHON_EXE%" "%ROOT%\fx-quant-stack\scripts\train_intraday_tcn.py" --pair %%P --timeframe M5 --feature-root "%FXSTACK_TRAIN_FEATURE_ROOT%" --label-root "%FXSTACK_TRAIN_LABEL_ROOT%" --out "%FXSTACK_TRAIN_ARTIFACT_ROOT%/%%P/intraday_tcn"
  if errorlevel 1 (
