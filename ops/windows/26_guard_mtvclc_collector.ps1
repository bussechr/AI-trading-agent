...
# AGENT: ROLE: guarded Windows supervisor and health probe for one sealed MTVCLC collector.
# AGENT: HANDSHAKE: exact prereg/output/source/key-file identity -> collection-only Python process.
# AGENT: ISOLATION: no secret value, signal/performance evaluation, runtime control, activation, or orders.
# AGENT: SIDE EFFECTS: StartOrResume creates supervision metadata/logs and runs only the sealed collector.
...
$CollectorPath = [IO.Path]::GetFullPath(
    (Join-Path $RepositoryRoot "tools\capture_ig_mt4_m1_activity.py")
)
$InspectorPath = [IO.Path]::GetFullPath(
    (Join-Path $RepositoryRoot "tools\check_mt4_tick_volume_collector_continuity.py")
)
...
function Write-JsonAndExit {
    ...
    $Payload["schema_version"] = "fxstack.mtvclc_collector_guard_status.v1"
...
if (-not ("FxStackNativeCommandLine" -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.ComponentModel;
using System.Runtime.InteropServices;

public static class FxStackNativeCommandLine
{
...
function Get-CollectorWriterGroups {
    ...
    foreach ($process in $processes) {
        ...
        try {
            [string[]]$arguments = [FxStackNativeCommandLine]::Split($commandLine)
        }
        catch {
            continue
        }
...
function New-BaseStatus {
    ...
    $t0Text = if ($Inspection.prospective_t0_utc_inclusive -is [DateTime]) {
        ([DateTime]$Inspection.prospective_t0_utc_inclusive).ToUniversalTime().ToString(
            "yyyy-MM-ddTHH:mm:ssZ",
            [Globalization.CultureInfo]::InvariantCulture
        )
    }
    else {
        [string]$Inspection.prospective_t0_utc_inclusive
    }
    $endText = if ($Inspection.prospective_end_utc_exclusive -is [DateTime]) {
        ([DateTime]$Inspection.prospective_end_utc_exclusive).ToUniversalTime().ToString(
            "yyyy-MM-ddTHH:mm:ssZ",
            [Globalization.CultureInfo]::InvariantCulture
        )
    }
    else {
        [string]$Inspection.prospective_end_utc_exclusive
    }
    return @{
        action = $Action
        ...
        prospective_t0_utc_inclusive = $t0Text
        prospective_end_utc_exclusive = $endText
        ...
        stdout_log = (Join-Path $ResolvedOutput "supervision\collector.stdout.log")
        stderr_log = (Join-Path $ResolvedOutput "supervision\collector.stderr.log")
    }
}

function Get-WriterLockState {
    $lockPath = Join-Path $ResolvedOutput "supervision\collector-writer.lock"
...
if ($Action -eq "Health") {
    ...
    if ([int64]$inspection.manifest_sequence -le 0 -or $null -eq $inspection.manifest_last_write_epoch) {
    ...
    $manifestAge = [Math]::Max(0.0, ([double]$now - [double]$inspection.manifest_last_write_epoch))
    $status["manifest_age_seconds"] = $manifestAge
    if ($manifestAge -gt $MaximumManifestAgeSeconds) {
        $status["status"] = "manifest_stale"
        Write-JsonAndExit $status 3
    }
...
$SupervisionDirectory = Join-Path $ResolvedOutput "supervision"
try {
...
try {
    ...
    if ($Action -eq "AdoptRunning") {
        ...
        Write-Output (@{
            schema_version = "fxstack.mtvclc_collector_guard_status.v1"
