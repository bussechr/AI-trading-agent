...
# AGENT: ROLE: exclusive Windows supervisor for the final v3 gap-ledger MTVCLC collector.
# AGENT: HANDSHAKE: runtime-pinned collector/producer sources + exact prereg/output/config/key-file path -> one collection-only writer group.
# AGENT: ISOLATION: no secret value, signal/outcome/performance, issuer, runtime, activation, or trade path.
# AGENT: SIDE EFFECTS: gap-v3 guard metadata and the exact collector child for StartOrResume only.
...
$RolloverMode = "refuse"
$StatusSchema = "fxstack.mtvclc_collector_guard_status.gap_v3.v1"
$InspectionSchema = "fxstack.mtvclc_collector_continuity_inspection.gap_v3.v1"
...
if (-not ("FxStackGapV3NativeCommandLine" -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.ComponentModel;
using System.Runtime.InteropServices;

public static class FxStackGapV3NativeCommandLine
{
...
function Get-CollectorWriterGroups {
    ...
    foreach ($process in $processes) {
        ...
        try {
            [string[]]$arguments = [FxStackGapV3NativeCommandLine]::Split($commandLine)
        }
        catch {
            continue
        }
...
if ($Action -eq "Health") {
    ...
    if ($writers.Count -eq 0) {
        if ($now -ge $end) {
            if (-not [bool]$inspection.guard_identity_present) {
                ...
                $status["reason"] = "immutable_gap_v3_guard_identity_missing"
    ...
    if ($lockState -ne "held") {
        ...
        $status["reason"] = "writer_not_owned_by_gap_v3_guard"
        Write-JsonAndExit $status 3
    }
    if ($now -ge $end) {
...
try {
    ...
    Write-JsonAndExit @{
        ...
        reason = "exclusive_gap_v3_writer_lock_unavailable"
...
try {
    ...
    if ($writers.Count -gt 0) {
        Write-JsonAndExit @{
            ...
            reason = "existing_gap_v3_writer_for_output_root"
            output_root = $ResolvedOutput
            writer_group_count = $writers.Count
