...
- `launch_all.bat live [EQUITY] [BRIDGE_PORT] [DASHBOARD_PORT]` (repo root): validated endpoint startup routed by strategy family. Every live path starts an inert authenticated bridge, proves broker attestation, captures the strategy-selected content binding, and only then permits runtime activation and execution egress.
...
- `wait_for_mt4_broker_ready.ps1`: read-only loopback gate between MT4 start and runtime activation; requires authenticated fresh heartbeat/ticks, exact broker mode/scope/Magic/venue/producer/symbol scope, positive free margin, and no unsupported symbols
- `21_start_runtime.bat`: runtime startup/readiness. `--binding-only` emits the selected strategy binding without spawning a runtime. Every live launch revalidates the captured binding immediately before reset/spawn; MTVCLC repeats admission verification inside runner startup.
...
Task start still requires deployment-owned live/live/armed, both shadow controls off, and expected account mode `demo`. The launcher proves the inert bridge and broker attestation before capturing the runtime-native or externally signed binding; runner repeats admission verification before authority activation, and durable queue admission still requires fresh matching broker attestation. Registration never starts MT4.
